...
ConsoleO:	dc.b "SEGA MEGA DRIVE " ; Hardware system ID (Console name)
...
GameInit:
	...
	@clearRAM:
        ...
        move.b	#1,(v_time_zone).l       ; Force Level To be Present TimeZone
MainGameLoop:
...
;		move	#$83,(v_vdp_buffer2).w
	...
	@nochg:
		jsr	LoadShieldArt			; Load shield art
		startZ80
...
SoundDriverLoad:
        ...
        lea (z80_ram).l,a1              ; load Z80 RAM space address
        move.w  #(Z80ROM_End-Z80ROM)-$01,d1     ; set repeat times
        move.w  #$0100,(z80_bus_request).l          ; request Z80 stop (ON)
        move.w  #$0100,(z80_reset).l          ; request Z80 reset (OFF)
        btst.b  #$00,(z80_bus_request).l            ; has the Z80 stopped yet?
...
SM_LoadZ80:
        ...
        lea (z80_ram+MuteSample).l,a1       ; load Z80 RAM space where the pointer is to be stored
        move.b  (a0)+,(a1)+             ; copy pointer over into Z80
        move.b  (a0)+,(a1)+             ; ''
        move.b  (a0)+,(a1)+             ; ''
        move.w  #$0000,(z80_reset).l          ; request Z80 reset (ON)
        moveq   #$7F,d1                 ; set repeat times
        dbf d1,*                    ; there's no way of checking for reset, so a manual delay is necessary
        move.w  #$0000,(z80_bus_request).l          ; request Z80 stop (OFF)
        move.w  #$0100,(z80_reset).l          ; request Z80 reset (OFF)
        rts                     ; return
...
Pal_R11B:	incbin	"palette\R11B.bin"
Pal_R11C:	incbin	"palette\R11C.bin"
Pal_R11D:	incbin	"palette\R11D.bin"
...
GM_Title:
		move.l	#$01010101,(f_levselcheat).w	; activate all cheats
		sfx	bgm_Stop,0,1,1 ; stop music
		bsr.w	ClearPLC
		bsr.w	PaletteFadeOut
		disable_ints
	...
	Tit_LoadText:
		...
		move.w	#0,(v_good_future).w ; unused variable
	...
	;	move.w	#0,d0
	;	bsr.w	EniDec
	;	lea	(Blk256_Title).l,a4 ; load GHZ 256x256 mappings
	;	lea	(v_256x256).l,a4
	;	move.w	#$2AFF,d0
	...
	;	dbf	d0,@loadchunks
		...
		bsr.w	NemDec
		moveq	#palid_GHZ,d0	; load title screen palette
		bsr.w	PalLoad1
...
GM_Level:
    cmpi.b    #$80,(v_time_zone).l     ; Is a Past Flag
	bne.s     @NotPast 
	move.b    #0,(v_time_zone).l
@NotPast:
	cmpi.b    #$81,(v_time_zone).l     ; Is a Present Flag
	bne.s     @NotPresent
	move.b    #1,(v_time_zone).l
@NotPresent:
	cmpi.b    #$82,(v_time_zone).l     ; Is a Future Flag
	bne.s     @NormalBootLevel
	move.b    #3,(v_time_zone).l            ; Force a good future
	cmpi.b    #1,(v_good_future).l     ; Is a Good Future Flag
	bne.s     @SetBadFuture
	bra.s     @NormalBootLevel
@SetBadFuture:
    move.b    #2,(v_time_zone).l
@NormalBootLevel:	   
;		cmpi.b  #2,(v_act).w
;		bne.s   @NormalBootLevel_2
;		move.b   #3,(v_time_zone).l            ; Force a good future
;	    cmpi.b   #1,(v_good_future).l     ; Is a Good Future Flag
;	    bne.s    @SetAct3BadFuture
;	    bra.s    @NormalBootLevel_2
;@SetAct3BadFuture:
;    move.b    #2,(v_time_zone).l
;@NormalBootLevel_2:
	...
	Level_NoMusicFade:
		...
		jsr	NemDec
		locVRAM	$7A00
		lea	(Nem_TitleCard_PPZ).l,a0; load title card patterns
		cmpi.b  #0,(v_zone).w  ;  is PPZ
		beq.s   @Load_Titlecard
        lea	(Nem_TitleCard_TTZ).l,a0; load title card patterns
		cmpi.b  #1,(v_zone).w  ;  is TTZ
		beq.s   @Load_Titlecard
		lea	(Nem_TitleCard_WWZ).l,a0; load title card patterns
		cmpi.b  #2,(v_zone).w  ;  is WWZ
		beq.s   @Load_Titlecard
		lea	(Nem_TitleCard_SSZ).l,a0; load title card patterns
		cmpi.b  #3,(v_zone).w  ;  is SSZ
		beq.s   @Load_Titlecard
		lea	(Nem_TitleCard_CCZ).l,a0; load title card patterns
		cmpi.b  #4,(v_zone).w  ;  is CCZ
		beq.s   @Load_Titlecard
		lea	(Nem_TitleCard_MMZ).l,a0; load title card patterns
	@Load_Titlecard:	
	    jsr	NemDec
		enable_ints
		moveq	#0,d0
...
Level_GetBgm:
	...
	Level_PlayBgm:
		...
		cmpi.b  #0,(v_time_zone).w
		bne.s   @playmusic
		lea	(MusicList_Past).l,a1 ; load past music playlist
	@playmusic:	
		move.b	(a1,d0.w),d0
		bsr.w	PlaySound	; play music
		cmpi.b  #1,(v_no_title_card).l
...
Level_TtlCardLoop:
	...
	Level_SkipTtlCard:
        ...
        move.b	#id_HUD,(v_obj_hud_score).w		; Load HUD score object
	    move.b	#id_HUD,(v_obj_hud_lives).w		; Load HUD lives object
	    move.b	#1,(v_obj_hud_lives+oSubtype).w
	    move.b	#id_HUD,(v_obj_hud_rings).w		; Load HUD rings object
	    move.b	#1,(v_obj_hud_rings+oSubtype2).w
	    jsr	LoadLifeIcon
	    move.b	#id_Monitor,(v_obj_hud_time_icon).w		; Load HUD time icon object
...
; ---------------------------------------------------------------------------
; Sonic CD Watterfall Generator
; ---------------------------------------------------------------------------
  include "_incObj/02 WatterfallGenerator.asm"      
; ---------------------------------------------------------------------------
; Sonic CD 3D Ramp
; ---------------------------------------------------------------------------
  include "_incObj/03 3DRamp.asm"
; ---------------------------------------------------------------------------
; Sonic CD Mosqui
; ---------------------------------------------------------------------------
  include "_incObj/04 Mosqui.asm"  
; ---------------------------------------------------------------------------
; Sonic CD Capsule
; ---------------------------------------------------------------------------
  include "_incObj/05 Capsule.asm"    
; ---------------------------------------------------------------------------
; Sonic CD Tonbom
; ---------------------------------------------------------------------------
  include "_incObj/06 Tonbon.asm"     
; ---------------------------------------------------------------------------
; Sonic CD Tidal Tempest Boss
; ---------------------------------------------------------------------------
  include "_incObj/07 TTZBoss.asm"    
; ---------------------------------------------------------------------------
; Sonic CD Tidal Tempest Boss Part 2
; ---------------------------------------------------------------------------
  include "_incObj/TTZBoss Part 2.asm"              
; ---------------------------------------------------------------------------
; Sonic CD Tidal Tempest Boss
; ---------------------------------------------------------------------------
  include "_incObj/19 TTZDoor.asm"        
; ---------------------------------------------------------------------------
; Sonic CD TimePost and Icon
; ---------------------------------------------------------------------------
  include "_incObj/TimePost and TimeIcon.asm"        
; ---------------------------------------------------------------------------
; Sonic CD Extended Camera
; ---------------------------------------------------------------------------
  include "_inc/SCD Extended Camera.asm"        
; ---------------------------------------------------------------------------
...
LevelDataLoad:
        ...
        lsl.w  #3,d1
        or.w  d1,d0
        lea  (LevelChunkAndBlockAddress).l,a2
        lea  (a2,d0.w),a2
; >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	;	move.l	a2,-(sp)
		;addq.l	#4,a2  ; skips the first 4 bytes
		movea.l	(a2)+,a0

		lea	(v_16x16).w,a1	; RAM address for 16x16 mappings
		move.w	#0,d0
		jsr	EniDec
		move.l	(a2)+,(ChunksInROMAddr).w

;		lea	(v_256x256).l,a4	; RAM address for
;		move.w	#$2AFF,d0
 ;   @loadchunks:
	;	move.l	(a0)+,(a4)+
	;	dbf	d0,@loadchunks
...
; ---------------------------------------------------------------------------
; Level	Zone Tiles Loading subroutine
; ---------------------------------------------------------------------------
LoadZoneTiles:
		moveq  #0,d0
        move.b  (v_zone).w,d0
...
LevelLayoutLoad2:
		...
		add.w	d2,d0
		add.w	d1,d0
		lea	(Level_Index_Past).l,a1
	    cmpi.b #0,(v_time_zone).w ;Is Past
		beq.s  @LoadTable
        lea	(Level_Index).l,a1
		cmpi.b #1,(v_time_zone).w ;Is Present
		beq.s  @LoadTable
	    lea	(Level_Index_Future).l,a1
		cmpi.b #2,(v_time_zone).w ;Is Future
		beq.s  @LoadTable
		lea	(Level_Index_GoodFuture).l,a1	
	@LoadTable:	
		move.w	(a1,d0.w),d0
		lea	(a1,d0.w),a1
		moveq	#0,d1
...
Map_Push:	include	"_maps\Pushable Blocks.asm"
		...
		include	"_incObj\34 CD_Card.asm"
...
@GotXVel:
	...
	move.w	#$8D,d0					; Play "I'm outta here" sound
	jmp     (PlaySample).l
...
ObjSonic_Main:
	jsr	Sonic_PanCamera			; Handle extended camera
...
@GivingUp:
	bsr.w	Sonic_LevelBound			; Handle level boundary collision
...
Nem_JapNames:	incbin	"artnem\Hidden Japanese Credits.bin"
		even
NemTonBon:      incbin	"artnem\NemTonBon.bin"
                even
; ---------------------------------------------------------------------------
; Sprite mappings - Sonic
; ---------------------------------------------------------------------------
Map_Sonic:
    incbin "sonic\map.bin"
	even
; ---------------------------------------------------------------------------
; Uncompressed graphics	- Sonic
; ---------------------------------------------------------------------------
Art_Sonic:	
     incbin	sonic\art.bin	; Sonic
	 even
; ---------------------------------------------------------------------------
; Uncompressed graphics	loading	array for Sonic
; ---------------------------------------------------------------------------
SonicDynPLC:
	incbin "sonic\dplc.bin"
	even	
; ---------------------------------------------------------------------------
...
Nem_LzBlock1:	incbin	"artnem\LZ 32x32 Block.bin"
		even
Nem_TTZEggman:  incbin  "artnem\TTZEggman.bin"
        even
Nem_TTZEggman2:  incbin  "artnem\TTZEggman2.bin"
        even		
Nem_TTZBlocksDoor: incbin "artnem\TTZBlocksDoor.bin"
        even				
; ---------------------------------------------------------------------------
...
; Compressed graphics - various
; ---------------------------------------------------------------------------
Nem_TitleCard:	incbin	"artnem\TitleCardBase.bin"
		even
Nem_TitleCard_PPZ:	incbin	artnem\TitleCards\PPZ.bin	; title cards
		even
Nem_TitleCard_TTZ:  incbin	artnem\TitleCards\TTZ.bin	; title cards
		even
Nem_TitleCard_WWZ:  incbin	artnem\TitleCards\WWZ.bin	; title cards
		even	
Nem_TitleCard_SSZ:  incbin	artnem\TitleCards\SSZ.bin	; title cards
		even
Nem_TitleCard_CCZ:  incbin	artnem\TitleCards\CCZ.bin	; title cards
		even
Nem_TitleCard_MMZ:  incbin	artnem\TitleCards\MMZ.bin	; title cards
		even							
Nem_Hud:	incbin	"hud\artbase.bin"
	        even
Nem_Lives:	incbin	"artnem\HUD - Life Counter Icon.bin"
		even
ArtUnc_LifeIcon:
	incbin	"hud/artlifeicon.bin"
	even		
...
; ---------------------------------------------------------------------------
; ---------------------------------------------------------------------------
; Palmtree Panic Act 1 Assets
;----------------------------------------------------------------------------
Kos_PPZ:	incbin	"artkos\8x8 - R11A.bin"	; GHZ secondary patterns
		even
Blk16_PPZ:	incbin	"map16\R11A.bin"
		even
Blk256_PPZ:	incbin	"map256_u\R11A.bin"
		even
; ---------------------------------------------------------------------------
; Palmtree Panic Act 1 Past Assets
;----------------------------------------------------------------------------		
Kos_PPZ_Past:	incbin	"artkos\8x8 - R11B.bin"	; GHZ secondary patterns
		even
Blk16_PPZ_Past:	incbin	"map16\R11B.bin"
		even
Blk256_PPZ_Past:	incbin	"map256_u\R11B.bin"
		even		
; ---------------------------------------------------------------------------
; Palmtree Panic Act 1 Good Future Assets
;----------------------------------------------------------------------------		
Kos_PPZ_GF:	incbin	"artkos\8x8 - R11C.bin"	; GHZ secondary patterns
		even
Blk16_PPZ_GF:	incbin	"map16\R11C.bin"
		even
Blk256_PPZ_GF:	incbin	"map256_u\R11C.bin"
		even		
; ---------------------------------------------------------------------------
; Palmtree Panic Act 1 Bad Future Assets
;----------------------------------------------------------------------------		
Kos_PPZ_BF:	incbin	"artkos\8x8 - R11D.bin"	; GHZ secondary patterns
		even
Blk16_PPZ_BF:	incbin	"map16\R11D.bin"
		even
Blk256_PPZ_BF:	incbin	"map256_u\R11D.bin"
		even										
; ---------------------------------------------------------------------------
; Palmtree Panic Act 2 Assets
;----------------------------------------------------------------------------		
Kos_PPZ_2:	incbin	"artkos\8x8 - R12A.bin"	; GHZ secondary patterns
		even
Blk16_PPZ_2:	incbin	"map16\R12A.bin"
		even
Blk256_PPZ_2:	incbin	"map256_u\R12A.bin"
		even
; ---------------------------------------------------------------------------
Blk16_Title:	incbin	"map16\Title.bin"
		even
Nem_Title:	incbin	"artnem\8x8 - GHZ1.bin"	; GHZ primary patterns
		even		
Blk256_Title:	incbin	"map256_u\Title.bin"
		even
Blk16_LZ:	incbin	"map16\R43C.bin"
		even
Kos_LZ:		incbin	"artkos\8x8 - R43C.bin"	; LZ primary patterns
		even
Blk256_LZ:	incbin	"map256_u\R43C.bin"
		even
Blk16_MZ:	incbin	"map16\MZ.bin"
		even
Kos_MZ:		incbin	"artkos\8x8 - MZ.bin"	; MZ primary patterns
		even
...
Blk16_SLZ:	incbin	"map16\SLZ.bin"
		even
Kos_SLZ:	incbin	"artkos\8x8 - SLZ.bin"	; SLZ primary patterns
		even
Blk256_SLZ:	incbin	"map256_u\SLZ.bin"
		even
Blk16_SYZ:	incbin	"map16\SYZ.bin"
		even
Kos_SYZ:	incbin	"artkos\8x8 - SYZ.bin"	; SYZ primary patterns
		even
Blk256_SYZ:	incbin	"map256_u\SYZ.bin"
		even
Blk16_SBZ:	incbin	"map16\SBZ.bin"
		even
Kos_SBZ:	incbin	"artkos\8x8 - SBZ.bin"	; SBZ primary patterns
		even
...
Nem_EndStH:	incbin	"artnem\Ending - StH Logo.bin"
		even
Nem_3DRamp: incbin 	3dramp\art.bin
		even
Nem_Mosqui: incbin 	mosqui\art.bin
		even		
Nem_Flower_Capsule: incbin 	flowercapsule\art.bin
		even						
...
CollArray2:	incbin	collide\carray_r.bin	; rotated collision array
		even
AngleMap_s1:	incbin	collide\anglemap_s1.bin	; floor angle map
		even
CollArray1_s1:	incbin	collide\carray_n_s1.bin	; normal collision array
		even
CollArray2_s1:	incbin	collide\carray_r_s1.bin	; rotated collision array
		even
Col_GHZ:	incbin	"collide\GHZ.bin"	; GHZ index
		even
Col_PPZ_Past:	incbin	"collide\R11B.bin"	; GHZ index
		even		
Col_LZ:		incbin	"collide\R43C.bin"	; LZ index
		even
...
Level_Index:
		...
		dc.w Level_GHZ2-Level_Index, Level_PPZbg_2-Level_Index, byte_68E3C-Level_Index
...
Level_Index_Past
		; GHZ
		dc.w Level_PPZ1_Past-Level_Index_Past, Level_PPZ1BG_Past-Level_Index_Past, byte_68D70-Level_Index_Past
		dc.w Level_GHZ2-Level_Index_Past, Level_PPZbg_2-Level_Index_Past, byte_68E3C-Level_Index_Past
		dc.w Level_GHZ3-Level_Index_Past, Level_GHZbg-Level_Index_Past, byte_68F84-Level_Index_Past
		dc.w byte_68F88-Level_Index_Past, byte_68F88-Level_Index_Past, byte_68F88-Level_Index_Past
		; LZ
		dc.w Level_LZ1-Level_Index_Past, Level_LZbg-Level_Index_Past, byte_69190-Level_Index_Past
		dc.w Level_LZ2-Level_Index_Past, Level_LZbg-Level_Index_Past, byte_6922E-Level_Index_Past
		dc.w Level_LZ3-Level_Index_Past, Level_LZbg-Level_Index_Past, byte_6934C-Level_Index_Past
		dc.w Level_SBZ3-Level_Index_Past, Level_LZbg-Level_Index_Past, byte_6940A-Level_Index_Past
		; MZ
		dc.w Level_MZ1-Level_Index_Past, Level_MZ1bg-Level_Index_Past, Level_MZ1-Level_Index_Past
		dc.w Level_MZ2-Level_Index_Past, Level_MZ2bg-Level_Index_Past, byte_6965C-Level_Index_Past
		dc.w Level_MZ3-Level_Index_Past, Level_MZ3bg-Level_Index_Past, byte_697E6-Level_Index_Past
		dc.w byte_697EA-Level_Index_Past, byte_697EA-Level_Index_Past, byte_697EA-Level_Index_Past
		; SLZ
		dc.w Level_SLZ1-Level_Index_Past, Level_SLZbg-Level_Index_Past, byte_69B84-Level_Index_Past
		dc.w Level_SLZ2-Level_Index_Past, Level_SLZbg-Level_Index_Past, byte_69B84-Level_Index_Past
		dc.w Level_SLZ3-Level_Index_Past, Level_SLZbg-Level_Index_Past, byte_69B84-Level_Index_Past
		dc.w byte_69B84-Level_Index_Past, byte_69B84-Level_Index_Past, byte_69B84-Level_Index_Past
		; SYZ
		dc.w Level_SYZ1-Level_Index_Past, Level_SYZbg-Level_Index_Past, byte_69C7E-Level_Index_Past
		dc.w Level_SYZ2-Level_Index_Past, Level_SYZbg-Level_Index_Past, byte_69D86-Level_Index_Past
		dc.w Level_SYZ3-Level_Index_Past, Level_SYZbg-Level_Index_Past, byte_69EE4-Level_Index_Past
		dc.w byte_69EE8-Level_Index_Past, byte_69EE8-Level_Index_Past, byte_69EE8-Level_Index_Past
		; SBZ
		dc.w Level_SBZ1-Level_Index_Past, Level_SBZ1bg-Level_Index_Past, Level_SBZ1bg-Level_Index_Past
		dc.w Level_SBZ2-Level_Index_Past, Level_SBZ2bg-Level_Index_Past, Level_SBZ2bg-Level_Index_Past
		dc.w Level_SBZ2-Level_Index_Past, Level_SBZ2bg-Level_Index_Past, byte_6A2F8-Level_Index_Past
		dc.w byte_6A2FC-Level_Index_Past, byte_6A2FC-Level_Index_Past, byte_6A2FC-Level_Index_Past
		zonewarning Level_Index_Past,24
		; Ending
		dc.w Level_End-Level_Index_Past, Level_GHZbg-Level_Index_Past, byte_6A320-Level_Index_Past
		dc.w Level_End-Level_Index_Past, Level_GHZbg-Level_Index_Past, byte_6A320-Level_Index_Past
		dc.w byte_6A320-Level_Index_Past, byte_6A320-Level_Index_Past, byte_6A320-Level_Index_Past
		dc.w byte_6A320-Level_Index_Past, byte_6A320-Level_Index_Past, byte_6A320-Level_Index_Past
Level_Index_Past_End		
Level_Index_Future:
		; GHZ
		dc.w Level_PPZ1_BF-Level_Index_Future, Level_PPZ1BG_BF-Level_Index_Future, byte_68D70-Level_Index_Future
		dc.w Level_GHZ2-Level_Index_Future, Level_PPZbg_2-Level_Index_Future, byte_68E3C-Level_Index_Future
		dc.w Level_GHZ3-Level_Index_Future, Level_GHZbg-Level_Index_Future, byte_68F84-Level_Index_Future
		dc.w byte_68F88-Level_Index_Future, byte_68F88-Level_Index_Future, byte_68F88-Level_Index_Future
		; LZ
		dc.w Level_LZ1-Level_Index_Future, Level_LZbg-Level_Index_Future, byte_69190-Level_Index_Future
		dc.w Level_LZ2-Level_Index_Future, Level_LZbg-Level_Index_Future, byte_6922E-Level_Index_Future
		dc.w Level_LZ3-Level_Index_Future, Level_LZbg-Level_Index_Future, byte_6934C-Level_Index_Future
		dc.w Level_SBZ3-Level_Index_Future, Level_LZbg-Level_Index_Future, byte_6940A-Level_Index_Future
		; MZ
		dc.w Level_MZ1-Level_Index_Future, Level_MZ1bg-Level_Index_Future, Level_MZ1-Level_Index_Future
		dc.w Level_MZ2-Level_Index_Future, Level_MZ2bg-Level_Index_Future, byte_6965C-Level_Index_Future
		dc.w Level_MZ3-Level_Index_Future, Level_MZ3bg-Level_Index_Future, byte_697E6-Level_Index_Future
		dc.w byte_697EA-Level_Index_Future, byte_697EA-Level_Index_Future, byte_697EA-Level_Index_Future
		; SLZ
		dc.w Level_SLZ1-Level_Index_Future, Level_SLZbg-Level_Index_Future, byte_69B84-Level_Index_Future
		dc.w Level_SLZ2-Level_Index_Future, Level_SLZbg-Level_Index_Future, byte_69B84-Level_Index_Future
		dc.w Level_SLZ3-Level_Index_Future, Level_SLZbg-Level_Index_Future, byte_69B84-Level_Index_Future
		dc.w byte_69B84-Level_Index_Future, byte_69B84-Level_Index_Future, byte_69B84-Level_Index_Future
		; SYZ
		dc.w Level_SYZ1-Level_Index_Future, Level_SYZbg-Level_Index_Future, byte_69C7E-Level_Index_Future
		dc.w Level_SYZ2-Level_Index_Future, Level_SYZbg-Level_Index_Future, byte_69D86-Level_Index_Future
		dc.w Level_SYZ3-Level_Index_Future, Level_SYZbg-Level_Index_Future, byte_69EE4-Level_Index_Future
		dc.w byte_69EE8-Level_Index_Future, byte_69EE8-Level_Index_Future, byte_69EE8-Level_Index_Future
		; SBZ
		dc.w Level_SBZ1-Level_Index_Future, Level_SBZ1bg-Level_Index_Future, Level_SBZ1bg-Level_Index_Future
		dc.w Level_SBZ2-Level_Index_Future, Level_SBZ2bg-Level_Index_Future, Level_SBZ2bg-Level_Index_Future
		dc.w Level_SBZ2-Level_Index_Future, Level_SBZ2bg-Level_Index_Future, byte_6A2F8-Level_Index_Future
		dc.w byte_6A2FC-Level_Index_Future, byte_6A2FC-Level_Index_Future, byte_6A2FC-Level_Index_Future
		zonewarning Level_Index_Future,24
		; Ending
		dc.w Level_End-Level_Index_Future, Level_GHZbg-Level_Index_Future, byte_6A320-Level_Index_Future
		dc.w Level_End-Level_Index_Future, Level_GHZbg-Level_Index_Future, byte_6A320-Level_Index_Future
		dc.w byte_6A320-Level_Index_Future, byte_6A320-Level_Index_Future, byte_6A320-Level_Index_Future
		dc.w byte_6A320-Level_Index_Future, byte_6A320-Level_Index_Future, byte_6A320-Level_Index_Future
Level_Index_Future_End				
Level_Index_GoodFuture:
		; GHZ
		dc.w Level_PPZ1_GF-Level_Index_GoodFuture, Level_PPZ1BG_GF-Level_Index_GoodFuture, byte_68D70-Level_Index_GoodFuture
		dc.w Level_GHZ2-Level_Index_GoodFuture, Level_PPZbg_2-Level_Index_GoodFuture, byte_68E3C-Level_Index_GoodFuture
		dc.w Level_GHZ3-Level_Index_GoodFuture, Level_GHZbg-Level_Index_GoodFuture, byte_68F84-Level_Index_GoodFuture
		dc.w byte_68F88-Level_Index_GoodFuture, byte_68F88-Level_Index_GoodFuture, byte_68F88-Level_Index_GoodFuture
		; LZ
		dc.w Level_LZ1-Level_Index_GoodFuture, Level_LZbg-Level_Index_GoodFuture, byte_69190-Level_Index_GoodFuture
		dc.w Level_LZ2-Level_Index_GoodFuture, Level_LZbg-Level_Index_GoodFuture, byte_6922E-Level_Index_GoodFuture
		dc.w Level_LZ3-Level_Index_GoodFuture, Level_LZbg-Level_Index_GoodFuture, byte_6934C-Level_Index_GoodFuture
		dc.w Level_SBZ3-Level_Index_GoodFuture, Level_LZbg-Level_Index_GoodFuture, byte_6940A-Level_Index_GoodFuture
		; MZ
		dc.w Level_MZ1-Level_Index_GoodFuture, Level_MZ1bg-Level_Index_GoodFuture, Level_MZ1-Level_Index_GoodFuture
		dc.w Level_MZ2-Level_Index_GoodFuture, Level_MZ2bg-Level_Index_GoodFuture, byte_6965C-Level_Index_GoodFuture
		dc.w Level_MZ3-Level_Index_GoodFuture, Level_MZ3bg-Level_Index_GoodFuture, byte_697E6-Level_Index_GoodFuture
		dc.w byte_697EA-Level_Index_GoodFuture, byte_697EA-Level_Index_GoodFuture, byte_697EA-Level_Index_GoodFuture
		; SLZ
		dc.w Level_SLZ1-Level_Index_GoodFuture, Level_SLZbg-Level_Index_GoodFuture, byte_69B84-Level_Index_GoodFuture
		dc.w Level_SLZ2-Level_Index_GoodFuture, Level_SLZbg-Level_Index_GoodFuture, byte_69B84-Level_Index_GoodFuture
		dc.w Level_SLZ3-Level_Index_GoodFuture, Level_SLZbg-Level_Index_GoodFuture, byte_69B84-Level_Index_GoodFuture
		dc.w byte_69B84-Level_Index_GoodFuture, byte_69B84-Level_Index_GoodFuture, byte_69B84-Level_Index_GoodFuture
		; SYZ
		dc.w Level_SYZ1-Level_Index_GoodFuture, Level_SYZbg-Level_Index_GoodFuture, byte_69C7E-Level_Index_GoodFuture
		dc.w Level_SYZ2-Level_Index_GoodFuture, Level_SYZbg-Level_Index_GoodFuture, byte_69D86-Level_Index_GoodFuture
		dc.w Level_SYZ3-Level_Index_GoodFuture, Level_SYZbg-Level_Index_GoodFuture, byte_69EE4-Level_Index_GoodFuture
		dc.w byte_69EE8-Level_Index_GoodFuture, byte_69EE8-Level_Index_GoodFuture, byte_69EE8-Level_Index_GoodFuture
		; SBZ
		dc.w Level_SBZ1-Level_Index_GoodFuture, Level_SBZ1bg-Level_Index_GoodFuture, Level_SBZ1bg-Level_Index_GoodFuture
		dc.w Level_SBZ2-Level_Index_GoodFuture, Level_SBZ2bg-Level_Index_GoodFuture, Level_SBZ2bg-Level_Index_GoodFuture
		dc.w Level_SBZ2-Level_Index_GoodFuture, Level_SBZ2bg-Level_Index_GoodFuture, byte_6A2F8-Level_Index_GoodFuture
		dc.w byte_6A2FC-Level_Index_GoodFuture, byte_6A2FC-Level_Index_GoodFuture, byte_6A2FC-Level_Index_GoodFuture
		zonewarning Level_Index_GoodFuture,24
		; Ending
		dc.w Level_End-Level_Index_GoodFuture, Level_GHZbg-Level_Index_GoodFuture, byte_6A320-Level_Index_GoodFuture
		dc.w Level_End-Level_Index_GoodFuture, Level_GHZbg-Level_Index_GoodFuture, byte_6A320-Level_Index_GoodFuture
		dc.w byte_6A320-Level_Index_GoodFuture, byte_6A320-Level_Index_GoodFuture, byte_6A320-Level_Index_GoodFuture
		dc.w byte_6A320-Level_Index_GoodFuture, byte_6A320-Level_Index_GoodFuture, byte_6A320-Level_Index_GoodFuture
Level_Index_GoodFuture_End		
Level_GHZ1:	incbin	"levels\R11A.bin"
		even
byte_68D70:	dc.b 0,	0, 0, 0
Level_GHZ2:	incbin	"levels\R12A.bin"
		even
byte_68E3C:	dc.b 0,	0, 0, 0
Level_GHZ3:	incbin	"levels\ghz3.bin"
		even
Level_GHZbg:	incbin	"levels\R11A_BG.bin"
		even
Level_PPZbg_2:	incbin	"levels\R12A_BG.bin"
		even		
Level_PPZ1_Past:	incbin	"levels\R11B.bin"
		even
Level_PPZ1BG_Past:	incbin	"levels\R11B_BG.bin"
		even				
Level_PPZ1_GF:	incbin	"levels\R11C.bin"
		even
Level_PPZ1BG_GF:	incbin	"levels\R11C_BG.bin"
		even				
Level_PPZ1_BF:	incbin	"levels\R11D.bin"
		even
Level_PPZ1BG_BF:	incbin	"levels\R11D_BG.bin"
		even								
...
ObjPosSBZPlatform_Index:
		...
		dc.w ObjPos_SBZ1pf1-ObjPos_Index, ObjPos_SBZ1pf2-ObjPos_Index
		dc.b $FF, $FF, 0, 0, 0,	0
ObjPos_GHZ1:	incbin	"objpos/R11A.bin"
            	even
