...
EntryPoint:
	btst	#6,(IO_C_CTRL).l			; Have the controller ports been initialized?
	beq.s	.DoInit					; If so, do start RAM clear
	cmpi.l	#'init',(v_init_flag).l			; Have we already initialized?
	beq.w	.GameInit				; If so, branch

.DoInit:
	lea	(v_unk_buffer).l,a6			; Clear RAM section starting at $FF1000
	moveq	#0,d7
...
.ClearRAM:
	...
	move.b	(HW_VERSION).l,d0			; Get hardware region
	andi.b	#$C0,d0
	move.b	d0,(v_hw_version).l

	move.l	#'init',(v_init_flag).l			; Mark as done

.GameInit:
	bsr.w	InitVDP					; Initialize VDP
;	jsr		SoundDriver_2Load		; Initialize SFX Only Sound Driver	
    jsr     SoundDriverLoad			; Initialize Sound Driver
	bsr.w	InitJoypads				; Initialize joypads
	move.b  #1,(v_time_zone).l


	move.b	#8,(v_game_mode).w			; Set game mode to "level"
...
MainGameLoop:
	move.b	(v_game_mode).w,d0			; Go to the current game mode routine
	andi.w	#$1C,d0
    jsr	GameModes(pc,d0.w) ; jump to apt location in ROM
...
GameModes:
ptr_GM_Level:	bra.w	Level		; $00
ptr_GM_ST:    bra.w   Go_SoundTest	; $04
ptr_GM_Sega:	bra.w   Go_SegaScreen  ; $08
ptr_GM_Title:	bra.w   Go_TitleScreen  ; $0C
; -------------------------------------------------------------------------------
; Game modes jumps
; -------------------------------------------------------------------------------	
Go_SoundTest:
...
PalCycle_Title:
	lea	(PPZ_ProtoPalCyc1).l,a0			; Prepare first palette data set
	subq.b	#1,(v_palcyc_timers).w			; Decrement timer
	bpl.s	.SkipCycle1				; If this cycle's timer isn't done, branch
	move.b	#7,(v_palcyc_timers).w			; Reset the timer

	moveq	#0,d0					; Get the current palette cycle frame
	move.b	(v_palcyc_steps).w,d0
...
.ApplyCycle1:
	move.b	d0,(v_palcyc_steps).w

	lsl.w	#3,d0					; Store the currnent palette cycle data in palette RAM
	lea	(v_palette+$6A).w,a1
	move.l	(a0,d0.w),(a1)+
	move.l	4(a0,d0.w),(a1)

.SkipCycle1:
	adda.w	#PPZ_ProtoPalCyc2-PPZ_ProtoPalCyc1,a0	; Prepare second palette data set
	subq.b	#1,(v_palcyc_timers+1).w		; Decrement timer
	bpl.s	.End					; If this cycle's timer isn't done, branch
	move.b	#5,(v_palcyc_timers+1).w		; Reset the timer

	moveq	#0,d0					; Get the current palette cycle frame
	move.b	(v_palcyc_steps+1).w,d0
	cmpi.b	#2,d0					; Should we wrap it back to 0?
...
PalCycle_Do:
	lea	(v_palcyc_timers).w,a5			; Prepare palette cycle variables
	lea	(v_palcyc_steps).w,a4

	lea	(PPZ_PalCyc_Script1).l,a1		; Cycle color 1
	lea	(PPZ_PalCyc_Colors1).l,a2
	bsr.s	PalCycle_OneColor

	lea	(PPZ_PalCyc_Script2).l,a1		; Cycle color 2
	lea	(PPZ_PalCyc_Colors2).l,a2
	bsr.s	PalCycle_OneColor

	lea	(PPZ_PalCyc_Script3).l,a1		; Cycle color 3
	lea	(PPZ_PalCyc_Colors3).l,a2
	bsr.s	PalCycle_OneColor

	lea	(PPZ_PalCyc_Script4).l,a1		; Cycle color 4
	lea	(PPZ_PalCyc_Colors4).l,a2
	bsr.s	PalCycle_OneColor

	lea	(PPZ_PalCyc_Script5).l,a1		; Cycle color 5
	lea	(PPZ_PalCyc_Colors5).l,a2
	bsr.s	PalCycle_OneColor

	lea	(PPZ_PalCyc_Script6).l,a1		; Cycle color 6
	lea	(PPZ_PalCyc_Colors6).l,a2
...
FadeFromBlack:
	...
	lea	(v_palette).w,a0
	move.b	(v_pal_fade_start).w,d0
	adda.w	d0,a0
	moveq	#0,d1					; Get palette fill value (black)
	move.b	(v_pal_fade_len).w,d0			; Get palette fill length
...
.Fade:
	move.b	#$12,(v_vint_routine).w			; VSync
	bsr.w	VSync
...
FadeColorsFromBlack:
	...
	lea	(v_palette).w,a0
	lea	(v_fade_palette).w,a1
	move.b	(v_pal_fade_start).w,d0
	adda.w	d0,a0
	adda.w	d0,a1
	move.b	(v_pal_fade_len).w,d0			; Get palette fade length

.Loop:
	bsr.s	FadeColorFromBlack			; Fade a color
	dbf	d0,.Loop				; Loop until finished

	cmpi.b	#1,(v_zone).l				; Are we in level ID 1 (Labyrinth Zone in Sonic 1)?
	bne.s	.End					; If not, branch

	moveq	#0,d0					; Get starting palette fade locations for water
	lea	(v_water_palette).w,a0
	lea	(v_water_fade_pal).w,a1
	move.b	(v_pal_fade_start).w,d0
	adda.w	d0,a0
	adda.w	d0,a1
	move.b	(v_pal_fade_len).w,d0			; Get palette fade length
...
Pal_FadeFrom:
FadeToBlack:
	move.w	#$3F,(v_pal_fade_start).w		; Set palette fade start and length
...
PalLoad1:
LoadFadePal:
	lea	(PaletteIndex).l,a1			; Get pointer to palette metadata
	lsl.w	#3,d0
	adda.w	d0,a1
...
CheckObjDespawn:
	...
	andi.w	#$FF80,d0
	move.w	(v_cam_fg_x).w,d1			; Get the camera's chunk position
	subi.w	#$80,d1
	andi.w	#$FF80,d1
...
.NoDraw:
	lea	(v_obj_respawns).l,a2			; Prepare object respawn table
	moveq	#0,d0
...
.Wait:
	tst.b	(v_vint_routine).w			; Has the V-INT routine run?
...
Player_LevelColOnGround:
	...
	move.b	d0,(v_primary_angle).w
	move.b	d0,(v_secondary_angle).w
	rts
...
Player_WalkFloor:
	...
	ext.w	d0
	add.w	d0,d3
	lea	(v_primary_angle).w,a4			; Get floor information from this sensor
	movea.w	#$10,a3
	move.w	#0,d6
	moveq	#$D,d5
	...
	lea	(v_secondary_angle).w,a4		; Get floor information from this sensor
	movea.w	#$10,a3
	move.w	#0,d6
	moveq	#$D,d5
	bsr.w	FindLevelFloor
...
Player_PickSensor:
	move.b	(v_secondary_angle).w,d2		; Use secondary angle
	cmp.w	d0,d1					; Is the primary sensor on the higher ground?
	ble.s	.GotAngle				; If not, branch
	move.b	(v_primary_angle).w,d2			; Use primary angle
...
.CheckStick:
	...
	rts
; -------------------------------------------------------------------------------
; Get level Chunk metadata at a position
; -------------------------------------------------------------------------------
GetChunkAddress:
    moveq    #0,d0
    move.b    (v_zone).l,d0
    ...
    add.w    d0,d0
    add.w    d0,d0
    add.b    (v_time_zone).l,d0
    add.w    d0,d0
    add.w    d0,d0
    cmpi.b   #2,(v_time_zone).l               ; is time zone 2 (future)?
    bne.s    .Load                            ; if not, go to ".Load"
	cmpi.b   #1,(v_good_future).l             ; if so, then is good future active?
	bne.s    .Load                            ; if not, go to ".Load"
	add.w    #4,d0                            ; if so, add 4 to d0    

.Load:
    move.l    .Table(pc,d0.w),(ChunkAddress).l
    rts
.Table:    
;       Past, Present, Future, GoodFuture
; Palmtree Panic Zone
        dc.l    PPZ_ChunksP
		dc.l    LevelChunks
		dc.l    PPZ_ChunksGF 
		dc.l    PPZ_ChunksBF ; Act 1

        dc.l    PPZ_2_ChunksP, PPZ_2_Chunks, PPZ_2_ChunksGF, PPZ_2_ChunksBF           ; Act 2
        dc.l    PPZ_3_Chunks, PPZ_3_Chunks, PPZ_3_Chunks, PPZ_3_Chunks           ; Act 3		
        dc.l    0, 0, 0, 0           ; Act 4
...
GetLevelBlock:
	...
	cmpi.b	#2,(v_zone).l				; Are we in Tidal Tempest?
...
.NotTTZ:
	...
	moveq    #0,d1                    ; clear entire long-word of d1

	lea	(v_lvl_layout).w,a1			; Get chunk that the block we want is in

	move.b	(a1,d0.w),d1
	beq.s	.Blank					; If it's a blank chunk, branch
	bmi.s	.LoopChunk				; If it's a loop chunk, branch

	cmpi.b	#5,(v_zone).l				; Are we in Stardust Speedway?
	beq.s	.SSZ					; If so, branch
	cmpi.b	#6,(v_zone).l				; Are we in Metallic Madness?
...
.NotMMZ:
	cmpi.b	#4,(v_zone).l				; Are we in Wacky Workbench?
...
.NotWWZ2:
	...
	tst.b	(v_display_low_plane).l			; Should things be on the high plane?
...
FindLevelFloor:
	...
	cmp.l	(ChunkAddress).l,d1				; Is it a blank block?
...
.IsSolid:
	movea.l	(v_collision_ptr).w,a2			; Get collision block ID
	move.b	(a2,d0.w),d0
	andi.w	#$FF,d0
	beq.s	.IsBlank				; If it's blank, branch

	lea	(ColAngleMap).l,a2			; Get collision angle
	move.b	(a2,d0.w),(a4)
...
; -------------------------------------------------------------------------------
; This seems to convert the collision array from an unknown "raw" format to
; its current format, overwriting the original. This function exists in
; Sonic 1, but wouldn't work on a standard ROM cartridge. However, due to
; levels in Sonic CD being loaded in Word RAM, this function performs as
; intended.
; -------------------------------------------------------------------------------
...
ConvColArray:
	...
	lea	(ColHeightMap).l,a1			; Source of "raw" collision array
	lea	(ColHeightMap).l,a2			; Destination of converted collision array
...
.RowLoop:
	...
	lea	(ColHeightMap).l,a1			; Convert widths
	lea	(ColWidthMap).l,a2
	bsr.s	.ConvToColBlocks
	lea	(ColHeightMap).l,a1			; Convert heights
	lea	(ColHeightMap).l,a2
...
Level:
	clr.w	(v_demo_mode).l				; Clear demo mode flag

	cmpi.b	#$7F,(v_got_time_stones).l		; Did we get all of the time stones?
	bne.s	.NotGoodFuture				; If not, branch
	tst.b	(v_time_attack_mode).l			; Are we in time attack mode?
	bne.s	.NotGoodFuture				; If not, branch
	move.b	#1,(v_good_future).l			; Force a good future

.NotGoodFuture:
	move.b	#0,(v_level_started).l			; Mark the level as not started yet
	clr.b	(v_vint_routine).w			; Reset V-INT routine ID
	clr.b	(v_use_player2).l			; Clear unused "use player 2" flag
	move.b	#0,(v_paused).w				; Clear pause flag

	bset	#0,(v_load_plc_flags).l			; Mark PLCs as loaded
	bne.s	.NoReset				; If they were loaded before, branch

	clr.b	(v_pal_fade_flags).l			; Mark palette fading as inactive
	clr.b	(v_last_checkpoint).l			; Reset checkpoint
	move.l	#5000,(v_next_score_1up).l		; Reset next score for 1-UP

	bsr.w	ResetRespawnTable			; Clear respawn table

	clr.b	(v_reset_lvl_flags).l			; Clear level reset flags
	clr.b	(v_good_future_flags).l			; Clear good future flags
	clr.l	(v_score).l				; Clear score

	move.b	#3,(v_life_count).l			; Reset life count to 3
	tst.b	(v_time_attack_mode).l			; Are we in time attack mode?
	beq.s	.NoReset				; If not, branch
	move.b	#1,(v_life_count).l			; Reset life count to 1

.NoReset:
	bset	#7,(v_game_mode).w			; Mark level as initializing
	bsr.w	ClearPLCs				; Clear PLCs

	tst.b	(v_entered_big_ring).l			; Have we entered a big ring before?
	bne.s	.EnteredBigRing				; If so, branch
	btst	#7,(v_time_zone).l			; Were we time travelling before?
	beq.s	.FadeToBlack				; If not, branch

	bset	#0,(v_pal_fade_flags).l			; Mark palette fading as active
	beq.s	.SkipFade				; If it was active before, branch

.EnteredBigRing:
	bsr.w	FadeToWhite				; Fade to white
	bclr	#0,(v_pal_fade_flags).l			; Mark palette fading as inactive

.SkipFade:
	clr.b	(v_time_warp_dir).w			; Reset time travel direction
	tst.w	(v_level_restart).l			; Was the level restart flag set?
	beq.w	.CheckNormalLoad			; If not, branch
	move.w	#0,(v_level_restart).l			; Clear level restart flag
	cmpi.b	#2,(v_act).l				; Are we in act 3?
	bne.s	.End					; If not, branch
	bclr	#7,(v_time_zone).l			; Clear time travel flag
...
.SkipFade2:
	cmpi.w	#2,(v_level_restart).l			; Were we going to the next level?
	bne.s	.CheckNoLives				; If not, branch
	move.w	#0,(v_level_restart).l			; Clear level restart flag
	move.b	#0,(v_pal_fade_flags).l			; Mark palette fading as inactive
	bra.s	.ClearPal				; Get out of here

.CheckNoLives:
	tst.b	(v_life_count).l			; Do we have any lives?
	bne.s	.CheckNormalLoad			; If so, branch
	move.b	#0,(v_load_plc_flags).l			; Mark PLCs as not loaded
	move.b	#0,(v_last_checkpoint).l		; Clear checkpoint
	move.b	#0,(v_reset_lvl_flags).l		; Clear level level flags
	move.b	#0,(v_pal_fade_flags).l			; Mark palette fading as inactive

.ClearPal:
	lea	(v_palette).w,a1			; Fill the palette with black
	move.w	#$80/4-1,d6
...
.ClearPalLoop:
	...
	move.b	#$C,(v_vint_routine).w			; Process the palette clear in V-INT
	bsr.w	VSync
	rts
...
.CheckNormalLoad:
	cmpi.w	#$800,(v_demo_timer).w			; Was a demo running?
	bne.s	.NormalLoad				; If not, branch
	move.w	#0,(v_demo_timer).w			; Reset demo timer
	clr.w	(v_demo_mode).l				; Clear demo mode flag
	move.b	#0,(v_pal_fade_flags).l			; Mark palette fading as inactive
	rts
...
.NormalLoad:
	...
	btst	#0,(v_pal_clear_flags).l		; Should we fill the palette with white?
...
.ClearPal2:
	lea	(v_palette).w,a1			; Fill the palette with black or white
	move.w	#$80/4-1,d6
...
.WaitPLC:
	move.b	#$C,(v_vint_routine).w			; VSync
	bsr.w	VSync
	bsr.w	ProcessPLCs				; Process PLCs
	...
	lea	(LevelPLCID).l,a2 ; Load Present PLC
	cmpi.b #0,(v_time_zone).l
	bne.s  .Load
	lea	(LevelPLCID_Past).l,a2 ; Load Past PLC
.Load:
	lea	(a2,d0.w),a2
	moveq	#0,d0
	move.b	(a2),d0
...
.LoadStdPLCs:
	...
	clr.b	(v_load_shield_art).l			; Reset shield art load flag
	clr.l	(v_flower_count).l			; Clear flower count

	lea	(v_obj_draw_queue).w,a1			; Clear object sprite draw queue
	moveq	#0,d0
	move.w	#$400/4-1,d1
...
.ClearObjSprites:
	...
	lea	(v_flower_pos_buffer).l,a1		; Clear flower position buffer and other misc. variables
	moveq	#0,d0
	move.w	#$A00/4-1,d1
...
.ClearFlowers:
	...
	lea	(v_objects).w,a1			; Clear object RAM
	moveq	#0,d0
	move.w	#$2000/4-1,d1
...
.ClearObjects:
	...
	lea	(v_unk_buffer2).l,a1			; Clear an unknown buffer
	moveq	#0,d0
	move.w	#$1000/4-1,d1
...
.ClearUnkBuffer:
	...
	lea	(v_misc_variables).w,a1			; Clear misc. variables
	moveq	#0,d0
	move.w	#$58/4-1,d1
...
.ClearMiscVars:
	...
	lea	(v_cam_fg_x).w,a1			; Clear camera RAM
	moveq	#0,d0
	move.w	#$100/4-1,d1
...
.ClearCamera:
	...
	move.l	#LevelChunks+$6C00,(v_demo_data_ptr).w	; Set demo data pointer (in DEMO11A, part of the chunk data
	...
	move.w	#$8ADF,(v_hint_counter).w		; Set H-INT counter to 233
	move.w	(v_hint_counter).w,(a6)

	move.w	#30,(v_air_left).l			; Set air timer
	...
	bset	#2,(v_scroll_flags).w			; Force draw a block column on the left side of the screen
...
.WaitPLC2:
	...
	tst.l	(v_plc_buffer).w			; Is the queue empty?
	bne.s	.WaitPLC2				; If not, wait

	bsr.w	LoadPlayer				; Load the player
	move.b	#$1C,(v_obj_hud_score).w		; Load HUD score object
	move.b	#$1C,(v_obj_hud_lives).w		; Load HUD lives object
	move.b	#1,(v_obj_hud_lives+oSubtype).w
	move.b	#$1C,(v_obj_hud_rings).w		; Load HUD rings object
	move.b	#1,(v_obj_hud_rings+oSubtype2).w
	bsr.w	LoadLifeIcon
	move.b	#$19,(v_obj_hud_time_icon).w		; Load HUD time icon object
	move.b	#$A,(v_obj_hud_time_icon+oSubtype).w

	bset	#1,(v_load_plc_flags).l			; Mark title card as loaded
...
;	move.b	(a1,d0.w),d0
;	jsr  	PlayFMSound	; play music
	move.b	#$3C,(v_obj_title_card).w		; Load the title card
	move.b	#1,(v_ctrl_locked).w			; Lock controls
	clr.b	(v_last_camera_plc).l			; Reset last camera PLC

.SkipTitleCard:
	move.w	#0,(v_player_held).w			; Clear controller data
	move.w	#0,(v_ctrl1_held).w
	move.w	#0,(v_ctrl2_held).w
	move.w	#0,(v_bored_timer).w			; Reset boredom timers
	move.w	#0,(v_p2_bored_timer).w
	move.b	#0,(v_unk_flag).l			; Clear unknown flag

	moveq	#0,d0
	tst.b	(v_reset_lvl_flags).l			; Was the level reset?
	bne.s	.SkipClear				; If so, branch
	move.w	d0,(v_ring_count).l			; Reset ring count
	move.l	d0,(v_time).l				; Reset time
	move.b	d0,(v_1up_flags).l			; Reset 1UP flags

.SkipClear:
	move.b	d0,(v_time_over).l			; Clear time over flag
	move.b	d0,(v_shield).l				; Clear shield flag
	move.b	d0,(v_invincible).l			; Clear invincible  flag
	move.b	d0,(v_speed_shoes).l			; Clear speed shoes flag
	move.b	d0,(v_time_warp_on).l			; Clear time warp flag
	move.w	d0,(v_debug_mode).l			; Clear debug mode flag
	move.w  #$01,(v_debug_mode_enabled).l
	move.w	d0,(v_level_restart).l			; Clear level restart flag
	move.w	d0,(v_frame_timer).l			; Reset frame timer
	move.b	d0,(v_reset_lvl_flags).l		; Clear level reset flags
	move.b	#1,(v_update_score).l			; Update the score in the HUD
	move.b	#1,(v_update_rings).l			; Update the ring count in the HUD
	move.b	#1,(v_update_time).l			; Update the time in the HUD
	move.b	#1,(v_update_lives).l			; Update the life counter in the HUD
	move.b	#$80,(v_update_rings).l			; Initialize the score in the HUD
	move.b	#$80,(v_update_score).l			; Initialize the score in the HUD

	move.w	#0,(v_s1_demo_data_index).w		; Clear demo data index (Sonic 1 leftover)
	move.w	#$202F,(v_pal_fade_start).w		; Set to fade palette lines 1-3

	jsr	JmpTo_LoadShieldArt			; Load shield art

	move.b	#1,(v_enable_display).l			; Set to enable display on palette fade
	bclr	#7,(v_time_zone).l			; Stop time travelling
...
.ChkPalFade:
	bclr	#0,(v_pal_clear_flags).l		; Did we fill the palette with white?
	bne.s	.FromWhite				; If so, branch
	bsr.w	FadeFromBlack				; Fade from black

.BeginLevel:
	bclr	#7,(v_game_mode).w			; Mark level as initialized
	move.b	#1,(v_level_started).l			; Mark level as started

; -------------------------------------------------------------------------------

Level_MainLoop:
	move.b	#8,(v_vint_routine).w			; VSync
	bsr.w	VSync

	cmpi.b	#6,(v_player+oRoutine).w		; Did the player die?
	bcc.s	.CheckPaused				; If so, branch
	tst.b	(v_ctrl_locked).w			; Are controls locked?
	bne.s	.CheckPaused				; If so, branch
	btst	#7,(v_ctrl1_pressed).w			; Was the start button pressed?
	beq.s	.CheckPaused				; If not, branch
	eori.b	#1,(v_paused).w				; Do pause/unpause

.CheckPaused:
	btst	#0,(v_paused).w				; Is the game paused?
	beq.w	.NotPaused				; If not, branch

	bsr.w	PauseMusic				; Pause music

	move.b	(v_ctrl1_pressed).w,d0			; Get pressed buttons
	tst.b	(v_time_attack_mode).l			; Are we in time attack mode?
	bne.s	.CheckReset				; If so, branch

	andi.b	#$70,d0					; Was A, B, or C pressed?
	beq.s	Level_MainLoop				; If not, branch
	subq.b	#1,(v_life_count).l			; Take away a life
	bpl.s	.GotLives				; If we haven't run out, branch
	clr.b	(v_life_count).l			; Cap lives at 0
...
.GotLives:
	...
	bsr.w	ResetRespawnTable			; Clear respawn table
	clr.b	(v_reset_lvl_flags).l			; Clear level reset flags
	move.w	#1,(v_level_restart).l			; Restart the level
	bra.s	.DoReset

.CheckReset:
	andi.b	#$70,d0					; Was A, B, or C pressed?
	beq.w	Level_MainLoop				; If not, branch
	clr.b	(v_life_count).l			; Set lives to 0

.DoReset:
	clr.b	(v_paused).w				; Clear pause flag
	clr.w	(v_demo_mode).l				; Clear demo mode flag
	clr.b	(v_last_checkpoint).l			; Clear checkpoint flag
	bra.w	Level					; Restart the level

.NotPaused:
	bsr.w	UnpauseMusic				; Unpause music

	addq.w	#1,(v_frame_timer).l			; Increment frame timer

	jsr	LevelObjManager				; Load level objects
	jsr	RunObjects				; Run objects

	cmpi.w	#$800,(v_demo_timer).w			; Has the demo time run out (not applicable here)?
	beq.w	Level					; If so, restart the level
	tst.w	(v_level_restart).l			; Is the level restarting?
	bne.w	Level					; If so, restart the level
	tst.w	(v_debug_mode).l			; Are we in debug mode?
	bne.s	.DoScroll				; If so, branch
	cmpi.b	#6,(v_player+oRoutine).w		; Is the player dead?
	bcs.s	.DoScroll				; If not, branch
	move.w	(v_cam_fg_y).w,(v_bottom_bound).w	; Set the bottom boundary of the level to wherever the camera is
	move.w	(v_cam_fg_y).w,(v_dest_btm_bound).w
...
.DrawObjects:
	...
	tst.w	(v_time_stop_timer).l			; Is the time stop timer active?
...
LoadPlayer:
	lea	(v_player).w,a1				; Player object
	moveq	#1,d0					; Set player object ID
	move.b	d0,oID(a1)
	tst.b	(v_reset_lvl_flags).l			; Was the level reset midway?
...
RestoreZoneFlowers:
	lea	(v_flower_count).l,a1			; Get flower count bsaed on time zone
	moveq	#0,d0
	move.b	(v_time_zone).l,d0
	bclr	#7,d0
	move.b	(a1,d0.w),d0
	beq.s	.End					; There are no flowers, exit

	subq.b	#1,d0					; Fix flower count for DBF
	lea	(v_dyn_objects).w,a2			; Dynamic object RAM
...
.Loop:
	...
	add.w	d2,d2
	moveq	#0,d3
	move.b	(v_time_zone).l,d3
	bclr	#7,d3
	lsl.w	#8,d3
	add.w	d3,d2
	lea	(v_flower_pos_buffer).l,a3		; Get flower position
	move.w	(a3,d2.w),oX(a2)
	move.w	2(a3,d2.w),oY(a2)

	adda.w	#oVarLen,a2				; Next object
...
LoadLevelCollision:
    ...
    lsl.w  #2,d1
    or.w  d1,d0
	move.l	LevelColIndex(pc,d0.w),(v_collision_ptr).w
	rts
...
; Palmtree Panic Zone Act 1
	dc.l	PPZ1_Collision_P				
	dc.l	LevelCollision				
	dc.l	PPZ1_Collision_BF
	dc.l	PPZ1_Collision_GF
; Palmtree Panic Zone Act 2					
	dc.l	PPZ2_Collision_P	
	dc.l	PPZ2_Collision
	dc.l	PPZ2_Collision_BF	
	dc.l	PPZ2_Collision_GF
; Palmtree Panic Zone Act 3	
	dc.l	PPZ3_Collision_GF
	dc.l	PPZ3_Collision_GF
	dc.l	PPZ3_Collision_GF
	dc.l	PPZ3_Collision_GF
...
HandleGlobalAnims:
	subq.b	#1,(v_logspike_anim_timer).l		; Decrement Sonic 1 spiked log animation timer
	bpl.s	.Rings					; If it hasn't run out, branch
	move.b	#$B,(v_logspike_anim_timer).l		; Reset animation timer
	subq.b	#1,(v_logspike_anim_frame).l		; Decrement frame
	andi.b	#7,(v_logspike_anim_frame).l		; Keep the frame in range

.Rings:
	subq.b	#1,(v_ring_anim_timer).l		; Decrement ring animation timer
	bpl.s	.Unknown				; If it hasn't run out, branch
	move.b	#7,(v_ring_anim_timer).l		; Reset animation timer
	addq.b	#1,(v_ring_anim_frame).l		; Increment frame
	andi.b	#3,(v_ring_anim_frame).l		; Keep the frame in range

.Unknown:
	subq.b	#1,(v_unk_anim_timer).l			; Decrement Sonic 1 unused animation timer
	bpl.s	.RingSpill				; If it hasn't run out, branch
	move.b	#7,(v_unk_anim_timer).l			; Reset animation timer
	addq.b	#1,(v_unk_anim_frame).l			; Increment frame
	cmpi.b	#6,(v_unk_anim_frame).l			; Keep the frame in range
	bcs.s	.RingSpill
	move.b	#0,(v_unk_anim_frame).l

.RingSpill:
	tst.b	(v_ring_spill_timer).l			; Has the ring spill timer run out?
	beq.s	.End					; If so, branch
	moveq	#0,d0					; Increment frame accumulator
	move.b	(v_ring_spill_timer).l,d0
	add.w	(v_ring_spill_accum).l,d0
	move.w	d0,(v_ring_spill_accum).l
	rol.w	#7,d0					; Set ring spill frame
	andi.w	#3,d0
	move.b	d0,(v_ring_spill_frame).l
	subq.b	#1,(v_ring_spill_timer).l		; Decrement ring spill timer
...
PlayLevelMusic:
	moveq	#0,d0                       ; Set Time Zone to 0
	moveq	#0,d1                       ; Set Zone to 0
	move.b	(v_time_zone).l,d0			; Get time zone
	bclr	#7,d0
	tst.b	(v_time_attack_mode).l			; Are we in time attack mode?
	bne.s	.Notfuture				; If so, branch
	cmpi.b	#2,d0					; Are we in the future?
	bne.s	.NotFuture				; If not, branch
	add.b	(v_good_future).l,d0			; Apply good future flag

.NotFuture:
	move.b	(v_zone).l,d1				; Send music play Sub CPU command
	add.w	d1,d1
	add.w	d1,d1
	add.w	d0,d1
	moveq	#0,d0
...
; -------------------------------------------------------------------------------
bgm_PPZ_P:	equ $81 ; PPZ Past
bgm_PPZ:	equ $82 ; PPZ Present
bgm_MZ:		equ $83	; SCD Title Theme
bgm_SLZ:	equ $84	; SCD Speed Shoes Theme
bgm_PPZ_GF:	equ $85 ; PPZ Good Future
bgm_PPZ_BF:	equ $86	; PPZ Bad Future
bgm_FZ:		equ $8D	; S1 Final Zone

MusicPlayCmds:
;           Past   Present  Future   Good Future	
	dc.b	bgm_PPZ_P, bgm_SSZP, bgm_PPZ_BF, bgm_PPZ_GF			; PPZ $00
	dc.b	$80, $12, $14, $13			; CCZ $01
	dc.b	$80, $15, $17, $16			; TTZ $02
	dc.b	$80, $18, $1A, $19			; QQZ $03
	dc.b	$80, $1B, $1D, $1C			; WWZ $04
	dc.b	$80, $1E, $20, $1F			; SSZ $05
	dc.b	$80, $21, $66, $22			; MMZ $06
...
;	jsr	PlayFMSound
	...
	bset	#7,(v_paused).w				; Set the music as paused
...
.Past:
	tst.b	(v_invincible).l			; Are we invincible?
	bne.s	.PauseMusic				; If so, pause the invincibility music
	tst.b	(v_speed_shoes).l			; Do we have speed shoes?
...
UnpauseMusic:
	bclr	#7,(v_paused).w				; Set the music as unpaused
...
loc_B10:
V_Interrupt:
	bset	#0,(GA_RESET).l				; Send Sub CPU IRQ2 request
	movem.l	d0-a6,-(sp)				; Save registers

	tst.b	(v_vint_routine).w			; Are we lagging?
	beq.s	VInt_Lag				; If so, branch

	move.w	(VDP_CTRL).l,d0	
	move.l	#$40000010,(VDP_CTRL).l			; Update VScroll
	move.l	(v_vscroll).w,(VDP_DATA).l

	btst	#6,(v_hw_version).l			; Is this a PAL console?
	beq.s	.NotPAL					; If not, branch
	move.w	#$700,d0				; Delay for a bit
	dbf	d0,*

.NotPAL:
	move.b	(v_vint_routine).w,d0			; Get V-INT routine ID
	move.b	#0,(v_vint_routine).w			; Mark V-INT as run
	andi.w	#$3E,d0
	move.w	VInt_Index(pc,d0.w),d0			; Run the current V-INT routine
	jsr	VInt_Index(pc,d0.w)

VInt_Finish:
	jsr	(sub_71B4C).l				; Update FM driver queues
	tst.b	(v_paused).w				; Is the game paused?
	bne.s	VInt_Done				; If so, branch
	bsr.w	RunBoredTimer				; Run boredom timer
	bsr.w	RunTimeWarp				; Run time warp timer

VInt_Done:
	addq.l	#1,(v_frame_count).l			; Increment frame counter
...
VInt_Lag:
	tst.b	(v_level_started).l			; Has the level started?
...
.NotPAL:
	move.w	#1,(v_hint_flag).w			; Set H-INT flag
	jsr	StopZ80					; Stop the Z80

	tst.b	(v_water_full).w			; Is water filling the screen?
	bne.s	.WaterPal				; If so, branch
	lea	(VDP_CTRL).l,a5				; DMA palette buffer
	move.l	#$94009340,(a5)
	move.l	#$96FD9580,(a5)
	move.w	#$977F,(a5)
	move.w	#$C000,(a5)
	move.w	#$80,(v_dma_cmd_cache).w
	move.w	(v_dma_cmd_cache).w,(a5)
	bra.s	.Done

.WaterPal:
	lea	(VDP_CTRL).l,a5				; DMA water palette buffer
	move.l	#$94009340,(a5)
	move.l	#$96FD9540,(a5)
	move.w	#$977F,(a5)
	move.w	#$C000,(a5)
...
.Done:
	move.w	(v_hint_counter).w,(a5)			; Update H-INT counter
...
VInt_S1SegaScr:						; Pointer to here is leftover from Sonic 1's
	...
	tst.w	(v_vint_timer).w			; Is the V-INT timer running?
	beq.w	.End					; If not, branch
	subq.w	#1,(v_vint_timer).w			; Decrement V-INT timer
...
VInt_Pause:
	cmpi.b	#$10,(v_game_mode).w			; Are we in the Sonic 1 special stage?
...
VInt_Level:
	...
	lea	(VDP_CTRL).l,a5				; DMA HScroll
	move.l	#$940193C0,(a5)
	move.l	#$96E69500,(a5)
	move.w	#$977F,(a5)
	move.w	#$7C00,(a5)
	move.w	#$83,(v_dma_cmd_cache).w
	move.w	(v_dma_cmd_cache).w,(a5)

	lea	(VDP_CTRL).l,a5				; DMA sprites
	move.l	#$94019340,(a5)
	move.l	#$96FC9500,(a5)
	move.w	#$977F,(a5)
	move.w	#$7800,(a5)
	...
	lea	(v_player).w,a0				; Load player sprite art
	bsr.w	LoadSonicDynPLC
	tst.b	(v_sonic_frame_changed).w
	beq.s	.NoArtLoad
	lea	(VDP_CTRL).l,a5
	move.l	#$94019370,(a5)
	move.l	#$96E49500,(a5)
	...
	move.w	#$83,(v_dma_cmd_cache).w
	move.w	(v_dma_cmd_cache).w,(a5)
	move.b	#0,(v_sonic_frame_changed).w

.NoArtLoad:
	jsr	JmpTo_LoadShieldArt			; Load shield art
	jsr	StartZ80				; Start the Z80

	movem.l	(v_cam_fg_x).w,d0-d7			; Draw level
	movem.l	d0-d7,(v_cam_x_fg_copy).l
	movem.l	(v_scroll_flags).w,d0-d1
	movem.l	d0-d1,(v_scroll_flags_copy).l
...
VInt_UnkE:
	...
	addq.b	#1,(v_vint_e_count).w			; Increment counter
	move.b	#$E,(v_vint_routine).w			; Set to run this routine again next VBlank
...
VInt_PalFade:
	...
	cmpi.b	#1,(v_enable_display).l			; Should we enable display?
	bne.s	.SetHIntCounter				; If not, branch
	addq.b	#1,(v_enable_display).l			; Set display as enabled

	move.w	(v_vdp_reg_1).w,d0			; Enable display
	ori.b	#$40,d0
	move.w	d0,(VDP_CTRL).l

.SetHIntCounter:
	move.w	(v_hint_counter).w,(a5)			; Set H-INT counter
	bra.w	ProcessPLCDec				; Process PLC art decompression
...
RunTimeWarp:
	tst.b	(v_player+oPlayerCharge).w		; Is the player charging?
	bne.s	.End					; If so, branch
	tst.w	(v_time_warp_timer).w			; Is the time warp timer active?
	beq.s	.End					; If not, branch	
	addq.w	#1,(v_time_warp_timer).w		; Increment time warp timer
...
RunBoredTimer:
	tst.w	(v_bored_timer).w			; Is the bored timer active?
	beq.s	.CheckP2Timer				; If not, branch
	addq.w	#1,(v_bored_timer).w			; Increment bored timer

.CheckP2Timer:
	tst.w	(v_p2_bored_timer).w			; Is the player 2 bored timer active?
	beq.s	.End					; If not, branch
	addq.w	#1,(v_p2_bored_timer).w			; Increment player 2 bored timer
...
RandomNumber:
	move.l	d1,-(sp)
	move.l	(v_rng_seed).w,d1			; Get RNG seed
...
.GotSeed:
	...
	move.w	d0,d1
	swap	d1
	move.l	d1,(v_rng_seed).w			; Update RNG seed
	move.l	(sp)+,d1
	rts
...
ReadJoypads:
	lea	(v_ctrl1_held).w,a0			; Read player 1 controller
	lea	(IO_A_DATA).l,a1
	bsr.s	ReadJoypad
	addq.w	#2,a1					; Read player 2 controller
...
InitVDP:
	lea	(VDP_CTRL).l,a0				; Get VDP ports
	lea	(VDP_DATA).l,a1

	lea	(VDPInitRegs).l,a2			; Prepare VDP registers
	moveq	#$13-1,d7
...
.InitRegs:
	...
	move.w	(VDPInitReg1).l,d0			; Set VDP register 1 cache
	move.w	d0,(v_vdp_reg_1).w
	move.w	#$8ADF,(v_hint_counter).w		; Set H-INT counter cache
...
.ClearCRAM:
	...
	clr.l	(v_vscroll).w				; Clear scroll values
	clr.l	(v_hscroll_full).w
...
ClearScreen:
	lea	(VDP_CTRL).l,a5				; Clear plane A
	move.w	#$8F01,(a5)
	move.l	#$940F93FF,(a5)
	move.w	#$9780,(a5)
	move.l	#$40000083,(a5)
...
.WaitPlane1Clear:
	...
	lea	(VDP_CTRL).l,a5				; Clear plane B
	move.w	#$8F01,(a5)
	move.l	#$940F93FF,(a5)
	move.w	#$9780,(a5)
	move.l	#$60000083,(a5)
...
.WaitPlane2Clear:
	...
	clr.l	(v_vscroll).w				; Reset scroll values
	clr.l	(v_hscroll_full).w

	lea	(v_sprites).w,a1			; Clear sprite cache
	moveq	#0,d0
...
.ClearSprites:
	...
	lea	(v_hscroll).w,a1			; Clear HScroll cache
	moveq	#0,d0
...
InitZ80_Dummy:
	move.w	#$100,(Z80_RESET).l			; Stop Z80 reset
	jsr	StopZ80(pc)				; Stop the Z80

	lea	(Z80_RAM).l,a1				; Prepare Z80 RAM
	move.b	#$F3,(a1)+				; di
	move.b	#$F3,(a1)+				; di
	...
	move.w	#0,(Z80_RESET).l			; Reset the Z80
	ror.b	#8,d0					; Wait
	move.w	#$100,(Z80_RESET).l			; Stop Z80 reset
...
UpdateFMQueues:
	...
	tst.b	(v_snd_queue_1).w			; Is queue 1 full?
	beq.s	.CheckQueue2				; If not, branch
	move.b	(v_snd_queue_1).w,(Z80_RAM+$1C09).l	; Update Z80 queue 1
	move.b	#0,(v_snd_queue_1).w			; Empty out queue 1

.CheckQueue2:
	tst.b	(v_snd_queue_2).w			; Is queue 2 full?
	beq.s	.CheckQueue3				; If not, branch
	move.b	(v_snd_queue_2).w,(Z80_RAM+$1C0A).l	; Update Z80 queue 2
	move.b	#0,(v_snd_queue_2).w			; Empty out queue 2

.CheckQueue3:
	tst.b	(v_snd_queue_3).w			; Is queue 3 full?
	beq.s	.End					; If not, branch
	move.b	(v_snd_queue_3).w,(Z80_RAM+$1C0B).l	; Update Z80 queue 3
	move.b	#0,(v_snd_queue_3).w			; Empty out queue 3
...
ShowVDPGraphics:
DrawTilemap:
	lea	(VDP_DATA).l,a6				; Prepare VDP data
...
NemDec:
	...
	lea	(VDP_DATA).l,a4				; (the VDP data port)
	bra.s	NemDecMain
...
NemDecMain:
	lea	(v_nem_dec_buffer).w,a1			; Prepare decompression buffer
...
LoadPLC:
	movem.l	a1-a2,-(sp)
	lea	(PLCIndex).l,a1				; Prepare PLC list index
...
ClearPLC:
ClearPLCs:
	lea	(v_plc_buffer).w,a2			; Clear PLC buffer
	moveq	#$80/4-1,d0
...
RunPLC_RAM:
ProcessPLCs:
	tst.l	(v_plc_buffer).w			; Is the PLC queue empty?
	beq.s	.End					; If so, branch
	tst.w	(v_plc_tile_cnt).w			; Is there a decompression in process already?
	bne.s	.End					; If so, branch

	movea.l	(v_plc_buffer).w,a0			; Get art pointer
	lea	(NemPCD_WriteRowToVDP).l,a3		; Write to VRAM
	lea	(v_nem_dec_buffer).w,a1			; Prepare Nemesis buffer
	move.w	(a0)+,d2
...
.NotXOR:
	...
	move.l	a3,(v_plc_nem_write).w
	move.l	d0,(v_plc_repeat).w
	move.l	d0,(v_plc_pixel).w
	move.l	d0,(v_plc_row).w
	move.l	d5,(v_plc_read).w
	move.l	d6,(v_plc_shift).w
...
ProcessPLCDec:
	tst.w	(v_plc_tile_cnt).w			; Is there anything to decompress?
	beq.w	ProcessPLCDec_Done			; If not, branch

; -------------------------------------------------------------------------------

ProcessPLCDec_Large:
	move.w	#18,(v_plc_proc_tile_cnt).w		; Decompress 18 tiles in this batch
	moveq	#0,d0					; Get VRAM address
	move.w	(v_plc_buffer+4).w,d0
	addi.w	#18*$20,(v_plc_buffer+4).w		; Advance VRAM address
...
ProcessPLCDec_Small:
	...
	tst.b	(v_scroll_lock).w			; Is scrolling locked?
	bne.s	ProcessPLCDec_Large			; If so, go with the large batch instead

	move.w	#3,(v_plc_proc_tile_cnt).w		; Decompress 3 tiles in this batch
	moveq	#0,d0					; Get VRAM address
	move.w	(v_plc_buffer+4).w,d0
...
ProcessPLCDec_Main:
	lea	(VDP_CTRL).l,a4				; Set VDP write command
	lsl.l	#2,d0
	lsr.w	#2,d0
	ori.w	#$4000,d0
	swap	d0
	...
	movea.l	(v_plc_nem_write).w,a3
	move.l	(v_plc_repeat).w,d0
	move.l	(v_plc_pixel).w,d1
	move.l	(v_plc_row).w,d2
	move.l	(v_plc_read).w,d5
	move.l	(v_plc_shift).w,d6
	lea	(v_nem_dec_buffer).w,a1

.Decomp:
	movea.w	#8,a5					; Store decompressed tile in VRAM
	bsr.w	NemPCD_NewRow
	subq.w	#1,(v_plc_tile_cnt).w			; Decrement total tile count
	beq.s	ProcessPLCDec_Pop			; If this art is finished being decompressed, branch
	subq.w	#1,(v_plc_proc_tile_cnt).w		; Decrement number of tiles left to decompress in this batch
	bne.s	.Decomp					; If we are not done, branch

	move.l	a0,(v_plc_buffer).w			; Update decompression registers
	move.l	a3,(v_plc_nem_write).w
	move.l	d0,(v_plc_repeat).w
	move.l	d1,(v_plc_pixel).w
	move.l	d2,(v_plc_row).w
	move.l	d5,(v_plc_read).w
	move.l	d6,(v_plc_shift).w
...
ProcessPLCDec_Pop:
	...
	lea	(v_plc_buffer).w,a0			; Pop a PLC queue entry out
...
GetPlayerObject:
	lea	(v_player).w,a6				; Player 1
	tst.b	(v_use_player2).l			; Are we using player 2?
	beq.s	.Done					; If not, branch
	lea	(v_player2).w,a6			; Player 2
...
LevelSizeLoad:
	moveq	#0,d0
	move.b	d0,(v_unused_f740).w			; Clear unused variables
	move.b	d0,(v_unused_f741).w
	move.b	d0,(v_unused_f746).w
	move.b	d0,(v_unused_f748).w
	move.b	d0,(v_event_routine).w			; Clear level event routine
	...
	;lea	(LevelSize).l,a0			; Prepare level size information
    lea	LevelSizeArray(pc,d0.w),a0 ; load level	boundaries
	
	move.w	(a0)+,d0				; Get unused word
	move.w	d0,(v_unused_f730).w
	move.l	(a0)+,d0				; Get left and right boundaries
	move.l	d0,(v_left_bound).w
	move.l	d0,(v_dest_left_bound).w
	move.l	(a0)+,d0				; Get top and bottom boundaries
	move.l	d0,(v_top_bound).w
	move.l	d0,(v_dest_top_bound).w
	move.w	(v_left_bound).w,d0			; Get left boundary + $240
	addi.w	#$240,d0
	move.w	d0,(v_left_bound3).w
	move.w	#$1010,(v_horiz_blk_crossed_flag).w	; Initialize horizontal block crossed flags
	move.w	(a0)+,d0				; Get camera Y center
	move.w	d0,(v_cam_y_center).w
	move.w	#320/2,(v_cam_x_center).w		; Get camera X center
...
;word_2027F8:
;        dc.w 4                      ; DATA XREF: InitCamera+16↑o
;        dc.w 0                      ; DATA XREF: ObjBossEggman_0_Routine8:ObjBossEggman_0_RoutineA↓o
;        dc.w $D97
;        dc.w 0
;        dc.w $310
;        dc.w $60
LevelSizeArray:
; Palmtree Panic Zone
             		dc.w $0004, $0000, $2897, $0000, $0710, $0060 ; Act 1
             		dc.w $0004, $0000, $2897, $0000, $0310, $0060 ; Act 2
word_2027F8:		dc.w $0004, $0000, $0D97, $0000, $0310, $0060 ; Act 3
...
;	lea	(LevelStartLoc).l,a1			; Prepare level start position
	moveq	#0,d1					; Get starting X position
	move.w	(a1)+,d1
	move.w	d1,(v_player+oX).w
	moveq	#0,d0					; Get starting Y position
	move.w	(a1),d0
	move.w	d0,(v_player+oY).w
...
.SkipXLeftBnd:
	move.w	(v_right_bound).w,d2			; Is the camera past the right boundary?
	cmp.w	d2,d1
	bcs.s	.SkipXRightBnd				; If not, branch
	move.w	d2,d1					; If so, cap it

.SkipXRightBnd:
	move.w	d1,(v_cam_fg_x).w			; Set camera X position
...
.SkipYTopBnd:
	cmp.w	(v_bottom_bound).w,d0			; Is the camera past the bottom boundary?
	blt.s	.SkipYBtmBnd				; If not, branch
	move.w	(v_bottom_bound).w,d0			; If so, cap it

.SkipYBtmBnd:
	move.w	d0,(v_cam_fg_y).w			; Set camera Y position

	bsr.w	InitLevelScroll				; Initialize level scrolling

	lea	(LoopChunks).l,a1			; Get loop chunks
	cmpi.w #$0001,(v_zone).l        ; Are we in PPZ Act 2
	bne.s .NormalLoad
	lea	(PPZ2_LoopChunks).l,a1			; Get loop chunks
	cmpi.w #$0002,(v_zone).l        ; Are we in PPZ Act 3
	bne.s .NormalLoad
	lea	(PPZ3_LoopChunks).l,a1			; Get loop chunks
.NormalLoad:
	move.l	(a1),(v_loop_chunks).w
	rts

; -------------------------------------------------------------------------------

StartLocArray:
		incbin	"level\level\startpos\R11A.bin"
		incbin	"level\level\startpos\R12A.bin"
		incbin	"level\level\startpos\R13C.bin"
		dc.w	$80,$A8
...
InitLevelScroll:
    cmpi.w  #$0001,(v_zone).l
	bne.s   .NoPPZ2
	jmp (PPZ2_IntScroll).l
.NoPPZ2:	
    cmpi.w  #$0002,(v_zone).l
	bne.s   .Load
	jmp (PPZ3_IntScroll).l
.Load:	
...
.No3DRamp:
	...
	swap	d0
	move.w	d0,(v_cam_bg2_y).w
	move.w	d0,(v_cam_bg3_y).w

	lsr.w	#1,d1					; Get background X positions
	move.w	d1,(v_cam_bg_x).w
	lsr.w	#2,d1
	move.w	d1,d2
	add.w	d2,d2
	add.w	d1,d2
	move.w	d2,(v_cam_bg3_x).w
	lsr.w	#1,d1
	move.w	d1,d2
	add.w	d2,d2
	add.w	d1,d2
	move.w	d2,(v_cam_bg2_x).w

	lea	(v_layer_speeds).l,a2			; Clear cloud speeds
	moveq	#$12,d6
...
.DoScroll:
	clr.w	(v_scroll_flags).w			; Clear scroll flags
	clr.w	(v_scroll_flags_bg).w
	clr.w	(v_scroll_flags_bg2).w
	clr.w	(v_scroll_flags_bg3).w
	...
	move.w	(v_cam_fg_y).w,(v_vscroll).w		; Update VScroll values
	move.w	(v_cam_bg_y).w,(v_vscroll+2).w
...
LevelDraw_UpdateBG:
	...
	lea	(v_scroll_flags_bg).w,a2		; Update background section 1
	lea	(v_cam_bg_x).w,a3
	lea	(v_lvl_layout+$40).w,a4
	move.w	#$6000,d2
	bsr.w	DrawLevelBG1

	lea	(v_scroll_flags_bg2).w,a2		; Update background section 2
	lea	(v_cam_bg2_x).w,a3
	bra.w	DrawLevelBG2
...
LevelDraw_Update:
	...
	lea	(v_scroll_flags_copy+2).l,a2		; Update background
	lea	(v_cam_x_bg_copy).l,a3
	lea	(v_lvl_layout+$40).w,a4
	move.w	#$6000,d2
	bsr.w	DrawLevelBG1

	lea	(v_scroll_flags_copy).l,a2		; Update foreground
	lea	(v_cam_x_fg_copy).l,a3
	lea	(v_lvl_layout).w,a4
	move.w	#$4000,d2
...
DrawLevelBG1:
	lea	(BGCameraSectIDs).l,a0			; Prepare background section camera IDs
	adda.w	#1,a0
...
.GotRowPos:
	move.w	(v_cam_bg_y).w,d0			; Get which camera the current block section is using
	add.w	d4,d0
	andi.w	#$FFF0,d0
	asr.w	#4,d0
	move.b	(a0,d0.w),d0
...
.GotScrollDir:
	move.w	(v_cam_bg_y).w,d0			; Prepare background section camera ID array
	andi.w	#$FFF0,d0
	asr.w	#4,d0
	suba.w	#1,a0
	lea	(a0,d0.w),a0
...
.CameraSects:
	dc.l	v_cam_x_bg_copy				; BG1 (static)
	dc.l	v_cam_x_bg_copy				; BG1 (dynamic)
	dc.l	v_cam_x_bg2_copy			; BG2 (dynamic)
	dc.l	v_cam_x_bg3_copy			; BG3 (dynamic)
...
GetBlockDataAbsXY:
	lea	(v_level_blocks).l,a1			; Prepare block data pointer
...
LevelDraw_Start:
	...
	lea	(v_cam_fg_x).w,a3			; Initialize foreground
	lea	(v_lvl_layout).w,a4
	move.w	#$4000,d2
	bsr.s	LevelDraw_StartFG

	lea	(v_cam_bg_x).w,a3			; Initialize background
	lea	(v_lvl_layout+$40).w,a4
	move.w	#$6000,d2
...
.Draw:
	...
	lea	(BGCameraSectIDs).l,a0
	adda.w	#1,a0
	move.w	(v_cam_bg_y).w,d0
	add.w	d4,d0
	andi.w	#$7F0,d0
...
BGCameraSectIDs:
	BG_CAM_SECT 16,  BG_STATIC			; Offscreen top row, required to be here

	BG_CAM_SECT 16,	 BG_STATIC			; Top clouds
	BG_CAM_SECT 32,	 BG_STATIC
	BG_CAM_SECT 48,	 BG_STATIC
	BG_CAM_SECT 64,	 BG_STATIC
	BG_CAM_SECT 64,	 BG_STATIC
	BG_CAM_SECT 64,	 BG_STATIC
	BG_CAM_SECT 32,	 BG_STATIC
	BG_CAM_SECT 48,	 BG_STATIC
	BG_CAM_SECT 48,	 BG_STATIC
	BG_CAM_SECT 32,	 BG_STATIC
	BG_CAM_SECT 160, BG_STATIC			; Top mountains
	BG_CAM_SECT 32,	 BG_STATIC			; Top bushes
	BG_CAM_SECT 224, BG_STATIC			; Top water

	BG_CAM_SECT 224, BG_STATIC			; Upside down water
	BG_CAM_SECT 32,  BG_STATIC			; Upside down bushes
	BG_CAM_SECT 160, BG_DYNAMIC_2			; Upside down mountains
	BG_CAM_SECT 16,  BG_STATIC			; Upside down clouds
	BG_CAM_SECT 32,  BG_STATIC
	BG_CAM_SECT 48,  BG_STATIC
	BG_CAM_SECT 64,  BG_STATIC
	BG_CAM_SECT 64,  BG_STATIC

	BG_CAM_SECT 64,  BG_STATIC			; Bottom clouds
	BG_CAM_SECT 32,  BG_STATIC
	BG_CAM_SECT 80,  BG_STATIC
	BG_CAM_SECT 32,  BG_STATIC
	BG_CAM_SECT 16,  BG_STATIC
	BG_CAM_SECT 160, BG_DYNAMIC_2			; Bottom mountains
	BG_CAM_SECT 32,  BG_STATIC			; Bottom bushes
	BG_CAM_SECT 144, BG_STATIC			; Bottom water
...
BGCameraSects:
	dc.l	v_cam_bg_x&$FFFFFF			; BG1 (static)
	dc.l	v_cam_bg_x&$FFFFFF			; BG1 (dynamic)
	dc.l	v_cam_bg2_x&$FFFFFF			; BG2 (dynamic)
	dc.l	v_cam_bg3_x&$FFFFFF			; BG3 (dynamic)
...
LoadLevelData:
    ...
    lea     (Level_PalleteID).l,a2
	moveq  #0,d0
    move.b  (v_zone).l,d0
    lsl.w  #2,d0
    
	moveq    #$7F,d1
    and.b    (v_time_zone).l,d1
    or.b    d1,d0
	
	lea	(a2,d0.w),a2
	moveq	#0,d0
	move.b   (a2)+,d0 ; get pallete ids
...
.ChkStdPLC:
	btst	#1,(v_load_plc_flags).l			; Was the title card marked as loaded?
	beq.s	.End					; If not, branch
...
LevelLayoutLoad:
LoadLevelLayout:
	lea	(v_lvl_layout).w,a3			; Clear layout RAM
	move.w	#$1FF,d1
	moveq	#0,d0
...
.Clear:
	...
	lea	(v_lvl_layout).w,a3			; Load foreground layout
	moveq	#0,d1
	bsr.w	.LoadPlane

	lea	(v_lvl_layout+$40).w,a3			; Load background layout
	moveq	#2,d1
...
.LoadPlane:
	...
	add.w	d2,d0
	add.w	d1,d0
	lea	(LevelLayoutIndex).l,a1
	cmpi.b  #0,(v_time_zone).l
	bne.s .Load
	lea	(LevelLayoutIndex_Past).l,a1
.Load:
	move.w	(a1,d0.w),d0
	lea	(a1,d0.w),a1
...
RunLevelEvents:
	...
	move.b	(v_zone).l,d0
	add.w	d0,d0
	move.w	LevelEventIndex(pc,d0.w),d0
	jsr	LevelEventIndex(pc,d0.w)
...
.NotGivingUp:
	...
	move.w	(v_dest_btm_bound).w,d0			; Is the bottom boundary shifting?
	sub.w	(v_bottom_bound).w,d0
	beq.s	.End					; If not, branch
	bcc.s	.MoveDown				; If it's scrolling down, branch

	neg.w	d1					; Set the speed to go up
	move.w	(v_cam_fg_y).w,d0			; Is the camera past the target bottom boundary?
	cmp.w	(v_dest_btm_bound).w,d0
	bls.s	.ShiftUp				; If not, branch
	move.w	d0,(v_bottom_bound).w			; Set the bottom boundary to be where the camera id
	andi.w	#$FFFE,(v_bottom_bound).w

.ShiftUp:
	add.w	d1,(v_bottom_bound).w			; Shift the boundary up
	move.b	#1,(v_btm_bound_shifting).w		; Mark as shifting

.End:
	rts

.MoveDown:
	move.w	(v_cam_fg_y).w,d0			; Is the camera near the bottom boundary?
	addq.w	#8,d0
	cmp.w	(v_bottom_bound).w,d0
...
.ShiftDown:
	add.w	d1,(v_bottom_bound).w			; Shift the boundary down
	move.b	#1,(v_btm_bound_shifting).w		; Mark as shifting
	rts
...
LevelEventIndex:
	dc.w	LevEvents_PPZ-LevelEventIndex		; PPZ
	dc.w	LevEvents_CCZ-LevelEventIndex		; CCZ
	dc.w	LevEvents_TTZ-LevelEventIndex		; TTZ
	dc.w	LevEvents_QQZ-LevelEventIndex		; QQZ
	dc.w	LevEvents_WWZ-LevelEventIndex		; WWZ
	dc.w	LevEvents_SSZ-LevelEventIndex		; SSZ
	dc.w	LevEvents_MMZ-LevelEventIndex		; MMZ
...
LevEvents_CCZ3:
	tst.b	(v_boss_flags).w			; Was the boss defeated?
...
LevEvents_WWZ:
	btst	#4,(v_boss_flags).w			; Is the boss active?
	bne.s	.BossActive				; If so, branch
	move.w	#$710,(v_dest_btm_bound).w		; Set default bottom boundary
	rts
...
.BossActive:
	...
	lea	(v_player).w,a1				; Check where the player is
...
.SetBound:
	...
	move.w	d0,(v_dest_btm_bound).w

	sub.w	(v_bottom_bound).w,d0			; Is the current bottom boundary near the target?
...
.CheckNearBound:
	...
	move.w	d1,(v_bottom_bound).w			; Update bottom boundary
...
LevEvents_QQZ3:
	...
	tst.b	(v_boss_flags).w			; Is the boss active?
	bne.s	.BossActive				; If so, branch
	move.w	#$320,(v_dest_btm_bound).w		; Set default bottom boundary

.End:
	rts

.BossActive:
	move.w	#$1F8,(v_bottom_bound).w		; Set bottom boundary for the boss
	move.w	#$1F8,(v_dest_btm_bound).w
	rts
...
.BossActive:
	...
	move.w	d0,(v_top_bound).w
	move.w	d0,(v_dest_top_bound).w
	move.w	d0,(v_bottom_bound).w
	move.w	d0,(v_dest_btm_bound).w
	rts
...
LevEvents_SSZ3:
	lea	(v_player).w,a1				; Have we reached Metal Sonic?
	cmpi.w	#$930,oX(a1)
	bge.s	.FoundMetalSonic			; If so, branch
	move.w	#$210,(v_dest_btm_bound).w		; If not, set default bottom boundary
	rts
...
.CheckNearBound:
	...
	move.w	d0,(v_bottom_bound).w			; Update bottom boundary
...
ObjectsLoad:
RunObjects:
	lea	(v_objects).w,a0			; Prepare objects
	moveq	#(v_objects_end-v_objects)/oVarLen-1,d7
...
.NextObj:
	lea	oVarLen(a0),a0				; Get next object
...
DrawObject:
	...
	cmpi.w	#$100,(v_cam_fg_y).w			; Are we near the top of the screen?
...
.ChkBottomWrap:
	cmpi.w	#$700,(v_cam_fg_y).w			; Are we near the bottom of the screen?
	bcs.s	.CheckY					; If not, branch
...
.CheckY:
	sub.w	(v_cam_fg_y).w,d3			; Is this object onscreen vertically?
	move.w	d3,d1
	add.w	d0,d1
	bmi.s	.End					; If not, branch
	move.w	d3,d1
...
.DrawObj:
	lea	(v_obj_draw_queue).w,a1			; Get the object draw queue for this object's priority level
...
DisplaySprite1:
DrawOtherObject:
	lea	(v_obj_draw_queue).w,a2			; Get the object draw queue for this object's priority level
	move.w	$18(a1),d0
	lsr.w	#1,d0
	andi.w	#$380,d0
	adda.w	d0,a2
...
ObjDrawCameras:
	...
	dc.l	v_cam_fg_x&$FFFFFF			; Relative to FG camera
	dc.l	v_cam_bg_x&$FFFFFF			; Relative to BG camera
	dc.l	v_cam_bg3_x&$FFFFFF			; Relative to BG3 camera

; -------------------------------------------------------------------------------
BuildSprites:
DrawObjects:
	lea	(v_sprites).w,a2			; Prepare sprite table cache
	moveq	#0,d5					; Prepare sprite counter
	lea	(v_obj_draw_queue).w,a4			; Prepare object draw queue
...
.NextLevel:
	...
	move.b	d5,(v_sprite_count).w			; Save sprite count
...
;	a2.l	- Sprite table cache pointer
; -------------------------------------------------------------------------------
...
ObjectIndex:
	...
	dc.l	ObjDelete				; 32 - Blank - Important to Boss!
	...
	dc.l    ObjTitleSonic           ; 41 - Sonic in TitleScreen
	dc.l    ObjBossEggman           ; 42 - PPZ Boss ($2A in OG MMD)
	dc.l    ObjBossBody             ; 43 - PPZ Boss Body ($2B in OG MMD)
	dc.l    ObjBossThigh            ; 44 - PPZ Boss Thigh ($2C in OG MMD)
	dc.l    ObjBossCalf             ; 45 - PPZ Boss Calf ($2D in OG MMD)
	dc.l    ObjBossFoot             ; 46 - PPZ Boss Foot ($2E in OG MMD)
	dc.l    ObjBossElbow            ; 47 - PPZ Boss Elbow ($2F in OG MMD)
	dc.l    ObjBossUpperArm         ; 48 - PPZ Boss Upper Arm ($30 in OG MMD)
	dc.l    ObjBossForearm          ; 49 - PPZ Boss Fore Arm ($31 in OG MMD)
	dc.l    ObjBossBumper           ; 4A - PPZ Boss Bumper ($32 in OG MMD)
	dc.l    TitleMenu               ; 4B - Free Slot
...
ObjSonic_ChkBoredom:
	lea	(v_bored_timer).w,a1			; Get boredom timer
...
.TimerGoing:
	...
	move.b	#1,(v_life_count).l			; Make it so a game over happens
...
.GotXVel:
	...
	move.w	#$89,d0					; Play "I'm outta here" sound
	jsr	PlayFMSound
...
ObjSonic:
	...
	cmpa.w	#v_player2,a0				; Are we the second player?
	beq.s	.NormalMode				; If so, branch
	tst.b	(v_debug_mode).l			; Are we in debug mode?
...
ObjSonic_MakeTimeWarpStars:
	...
	move.b	#3,(v_obj_timewarp_star1).w		; Load time warp stars
	move.b	#5,(v_obj_timewarp_star1+oAnim).w
	move.b	#3,(v_obj_timewarp_star2).w
	move.b	#6,(v_obj_timewarp_star2+oAnim).w
	move.b	#3,(v_obj_timewarp_star3).w
	move.b	#7,(v_obj_timewarp_star3+oAnim).w
	move.b	#3,(v_obj_timewarp_star4).w
...
.NotMini:
	...
	move.w	#$600,(v_sonic_top_speed).w		; Set physics values
...
ObjSonic_MakeWaterfallSplash:
	tst.b	(v_zone).l				; Are we in Palmtree Panic zone?
	bne.s	.End					; If not, branch

	move.b	(v_frame_timer+1).l,d0			; Are we on an odd numbered frame?
	andi.b	#1,d0
	bne.s	.End					; If so, branch
...
ObjSonic_ExtCamera:
	move.w	(v_cam_x_center).w,d1			; Get camera X center position
...
.SetPanVal:
	move.w	d1,(v_cam_x_center).w			; Update camera X center position
	rts
...
ObjSonic_Main:
	...
	tst.w	(v_debug_mode_enabled).l		; Is debug mode enabled?
	beq.s	.NoDebug				; If not, branch
	btst	#4,(v_ctrl1_pressed).w			; Was the B button pressed?
	beq.s	.NoDebug				; If not, branch
	move.b	#1,(v_debug_mode).l			; Enter debug mode
	rts

.NoDebug:
	tst.b	(v_ctrl_locked).w			; Are controls locked?
	bne.s	.CtrlLock				; If so, branch
	move.w	(v_ctrl1_held).w,(v_player_held).w	; Copy controller data
...
.CtrlLock:
	...
	clr.w	(v_time_warp_timer).w			; Disable time warping
...
.SkipControl:
	...
	move.b	(v_primary_angle).w,oPlayerPriAngle(a0)	; Update our angle buffers
	move.b	(v_secondary_angle).w,oPlayerSecAngle(a0)

	tst.b	(v_wind_tunnel_on).w			; Are we in a wind tunnel?
...
ObjSonic_Display:
	cmpi.w	#210,(v_time_warp_timer).w		; Are we about to time travel?
...
.SkipDisplay:
	...
	tst.b	(v_speed_shoes).l			; Is the speed shoes music playing?
	bne.s	.StopInvinc				; If so, branch
	tst.b	(v_boss_music_playing).l		; Is the boss music playing?
	bne.s	.StopInvinc				; If so, branch
...
.StopInvinc:
	move.b	#0,(v_invincible).l			; Stop invincibility

.NotInvincible:
	tst.b	(v_speed_shoes).l			; Do we have speed shoes?
	beq.s	.End					; If not, branch
	tst.w	oPlayerSpeed(a0)			; Is the speed shoes timer active?
	beq.s	.End					; If not, branch

	subq.w	#1,oPlayerSpeed(a0)			; Decrement speed shoes time
	bne.s	.End					; If it hasn't run out, branch

	move.w	#$600,(v_sonic_top_speed).w		; Return physics back to normal
	move.w	#$C,(v_sonic_acceleration).w
	move.w	#$80,(v_sonic_deceleration).w

	tst.b	(v_invincible).l			; Is the invincibility music playing?
...
.StopSpeedShoes:
	move.b	#0,(v_speed_shoes).l			; Stop speed shoes
...
ObjSonic_RecordPos:
	move.w	(v_sonic_record_index).w,d0		; Get pointer to current position buffer index
	...
	addq.b	#4,(v_sonic_record_index+1).w		; Advance position buffer index
	rts
...
.HasWater:
	cmpi.b	#1,(v_act).l				; Are we in act 2 of Tidal Tempest?
	bne.s	.NotAct2				; If not, branch
	cmpi.w	#$C8,oX(a0)				; Are we in the wrapping section?
	bcs.s	.End					; If so, branch

.NotAct2:
	move.w	(v_water_height).w,d0			; Are we in the water?
	cmp.w	oY(a0),d0
	...
	move.b	#$21,(v_obj_bubbles).w			; Create bubbles that come out of our mouth
	move.b	#$81,(v_obj_bubbles+oSubtype).w

	move.w	#$300,(v_sonic_top_speed).w		; Set to water physics
...
.End2:
;   jsr GetChunkAddress ; This line doesnt crash
;	jsr LevelLayoutLoad ; This line crashes
;   jsr Reload_Level_PLC ; This line crashes x2
	rts
...
.NotGivingUp:
	...
	bsr.w	Player_LevelColOnGround			; Handle level collision
...
ObjSonic_Handle3DRamp:
	cmpi.b	#1,(v_time_zone).l			; Are we in the present?
	bne.s	.End					; If not, branch
	tst.w	(v_zone).l				; Are we in Palmtree Panic act 1?
	bne.s	.End					; If not, branch
	...
	lea	(.ChunkList).l,a2			; Get list of 3D ramp chunks
...
ObjSonic_MoveGround:
	move.w	(v_sonic_top_speed).w,d6		; Get top speed
	move.w	(v_sonic_acceleration).w,d5		; Get acceleration
	move.w	(v_sonic_deceleration).w,d4		; Get deceleration

	tst.b	(v_jump_only).w				; Can we only jump?
	bne.w	.CalcXYVels				; If so, branch
	tst.w	oPlayerMoveLock(a0)			; Is our movement locked temporarily?
	bne.w	.ResetScreen				; If so, branch

	btst	#2,(v_player_held).w			; Are we holding left?
	beq.s	.NotLeft				; If not, branch
	bsr.w	ObjSonic_MoveGndLeft			; Move left

.NotLeft:
	btst	#3,(v_player_held).w			; Are we holding right
...
.CheckCharge:
	move.b	(v_look_mode).w,d0			; Get double tap timer
	andi.b	#$F,d0
	beq.s	.DblTapNotInit				; If it's not active, branch
	addq.b	#1,(v_look_mode).w			; Increment timer
	andi.b	#$CF,(v_look_mode).w			; Cap the timer properly

.DblTapNotInit:
	btst	#7,(v_look_mode).w			; Is the look up flag set?
	bne.w	.LookUp					; If so, branch
	btst	#6,(v_look_mode).w			; Is the look down flag set?
	bne.w	.CheckForSpindash			; If so, branch

	btst	#1,(v_player_held).w			; Is down being held?
	bne.w	.CheckForSpindash			; If so, branch
	andi.b	#$F,(v_look_mode).w			; Clear out look flags
	beq.s	.CheckUpPress				; If the double tap timer wasn't set, branch

	btst	#0,(v_player_pressed).w			; Have we double tapped up?
	beq.s	.CheckUpHeld				; If not, branch
	bset	#7,(v_look_mode).w			; Set the look up flag
	bra.w	.Settle					; Settle movement

.CheckUpPress:
	btst	#0,(v_player_pressed).w			; Have we just pressed up?
	beq.w	.CheckUpHeld				; If not, branch
	move.b	#1,(v_look_mode).w			; Set the double tap timer to be active
	bra.w	.Settle					; Settle movement

.CheckUpHeld:
	btst	#0,(v_player_held).w			; Are we holding up?
...
.Peelout:
	...
	move.w	(v_sonic_top_speed).w,d1		; Get max charge speed (top speed * 2)
	move.w	d1,d2
	asl.w	#1,d1
	tst.b	(v_speed_shoes).l			; Do we have speed shoes?
...
.CheckStartCharge:
	move.b	(v_player_pressed).w,d0			; Did we press A, B, or C while we were holding up?
	andi.b	#$70,d0
	beq.s	.DontCharge				; If not, branch

	move.b	#1,oPlayerCharge(a0)			; Set the look double tap timer to be active
	move.w	#$BE,d0					; Play charge sound
	jsr	PlayFMSound
...
.UnleashPeelout:
	move.b	#0,oPlayerCharge(a0)			; Stop charging
	move.w	#$BC,d0					; Play charge release sound
	jsr	PlayFMSound
	bra.w	.ResetScreen				; Reset screen position
...
.LookUp:
	...
	move.b	#7,oAnim(a0)				; Set animation to looking up animation
	cmpi.w	#$C8,(v_cam_y_center).w			; Has the screen scrolled up all the way?
	beq.w	.Settle					; If so, branch
	addq.w	#2,(v_cam_y_center).w			; Move the screen up
	bra.w	.Settle					; Settle movement
...
.CheckForSpindash:
	...
	andi.b	#$F,(v_look_mode).w			; Clear out the look flags
	beq.s	.CheckDownPress				; If the double tap timer wasn't set, branch

	btst	#1,(v_player_pressed).w			; Have we double tapped down?
	beq.s	.CheckSpindash				; If not, branch
	bset	#6,(v_look_mode).w			; Set the look down flag
	bra.w	.Settle					; Settle movement

.CheckDownPress:
	btst	#1,(v_player_pressed).w			; Have we just pressed down?
	beq.s	.CheckSpindash				; If not, branch
	move.b	#1,(v_look_mode).w			; Set the double tap timer to be active
	bra.w	.Settle					; Settle movement

.CheckSpindash:
	btst	#1,(v_player_held).w			; Are we holding down?
	...
	move.b	(v_player_pressed).w,d0			; Did we press A, B, or C while we were holding down?
	andi.b	#$70,d0
...
.Duck:
	...
	cmpi.w	#8,(v_cam_y_center).w			; Has the screen scrolled dowm all the way?
	beq.s	.Settle					; If so, branch
	subq.w	#2,(v_cam_y_center).w			; Move the screen down
	bra.s	.Settle					; Settle movement

; -------------------------------------------------------------------------------

.ResetScreen:
	cmpi.w	#$60,(v_cam_y_center).w			; Is the screen centered?
	bne.s	.CheckIncShift				; If not, branch

	move.b	(v_look_mode).w,d0			; Get look double tap timer
	andi.b	#$F,d0
	bne.s	.Settle					; If it's active, branch
	move.b	#0,(v_look_mode).w			; Reset double tap timer and charge lock flags
	bra.s	.Settle					; Settle movement

.CheckIncShift:
	bcc.s	.DecShift				; If the screen needs to move back down, branch
	addq.w	#4,(v_cam_y_center).w			; Move the screen back up

.DecShift:
	subq.w	#2,(v_cam_y_center).w			; Move the screen back down

; -------------------------------------------------------------------------------

.Settle:
	move.b	(v_player_held).w,d0			; Are we holding left or right?
	andi.b	#$C,d0
...
.SetGVel2:
	...
	move.w	#$A4,d0					; Play skidding sound
	jsr	PlayFMSound
...
ObjSonic_MoveRoll:
	move.w	(v_sonic_top_speed).w,d6		; Get top speed (multiplied by 2)
	asl.w	#1,d6
	move.w	(v_sonic_acceleration).w,d5		; Get acceleration (divided by 2)
	asr.w	#1,d5
	move.w	(v_sonic_deceleration).w,d4		; Get deceleration (divided by 4)
	asr.w	#2,d4
...
ObjSonic_MoveAir:
	move.w	(v_sonic_top_speed).w,d6		; Get top speed
	move.w	(v_sonic_acceleration).w,d5		; Get acceleration (multiplied by 2)
	asl.w	#1,d5
...
ObjSonic_LevelBound:
	...
	move.w	(v_left_bound).w,d0			; Have we crossed the left boundary?
	addi.w	#16,d0
	cmp.w	d1,d0
	bhi.s	.Sides					; If so, branch

	move.w	(v_right_bound).w,d0			; Get right boundary
	addi.w	#320-16,d0
	tst.b	(v_boss_fight).w			; Are we in a boss fight?
...
.CheckBottom:
	move.w	(v_bottom_bound).w,d0			; Have we crossed the bottom boundary?
	addi.w	#224,d0
	cmp.w	$C(a0),d0
...
.CanJump:
	move.b	(v_player_held).w,d0			; Are we holding up or down?
	andi.b	#3,d0
...
.NotUpDown:
	move.b	(v_player_pressed).w,d0			; Have we pressed A, B, or C?
	andi.b	#$70,d0
	beq.w	.End					; If not, branch
...
.GotAngle:
	...
	clr.b	(v_look_mode).w				; Reset look double tap timer and flags

	move.w	#$A0,d0					; Play jump sound
	jsr	PlaySound
...
.GotCapVel:
	...
	move.b	(v_player_held).w,d0			; Are we holding A, B, or C?
	andi.b	#$70,d0
	bne.s	.End					; If so, branch
...
Player_LevelColInAir:
	...
	move.b	d0,(v_angle_buffer).l			; Update debug angle buffers
	subi.b	#$20,d0
	move.b	d0,(v_angle_normal_buf).l
	andi.b	#$C0,d0
	move.b	d0,(v_quadrant_normal_buf).l
...
.NotJumping:
	...
	move.w	#0,(v_chain_bonus_counter).w		; Clear chain bonus counter
	rts
...
ObjSonic_HurtChkLand:
	move.w	(v_bottom_bound).w,d0			; Have we gone past the bottom level boundary?
	addi.w	#224,d0
	cmp.w	oY(a0),d0
...
ObjSonic_DeadChkGone:
	...
	clr.b	(v_update_time).l			; Stop the level timer
	addq.b	#1,(v_update_lives).l			; Decrement life counter
	subq.b	#1,(v_life_count).l
...
.CapLives:
	...
	move.b	#0,(v_life_count).l			; Set lives to 0
...
.LoadGameOver:
	...
	tst.b	(v_life_count).l			; Do we still have lives left?
	beq.s	.End					; If not, branch
...
ObjSonic_Restart:
	...
	move.w	#1,(v_level_restart).l			; Set to restart the level
	...
	clr.l	(v_flower_count).l			; Reset flower count

	tst.b	(v_last_checkpoint).l			; Have we hit a checkpoint?
	bne.s	.Skip					; If so, branch
	cmpi.b	#1,(v_time_zone).l			; Are we in the present?
	bne.s	.Skip					; If not, branch
	bclr	#1,(v_load_plc_flags).l			; Set to reload the title card upon restarting

.Skip:
	move.w	#$E,d0					; Set to fade out music

	tst.b	(v_life_count).l			; Are we out of lives?
	...
	move.b	#1,(v_reset_lvl_flags).l		; Mark as restarted midway through the level
	bra.s	.SendCmd				; Continue setting the fade out command

.ClearFlag:
	clr.b	(v_reset_lvl_flags).l			; Set to fully restart the level

.SendCmd:
	bra.w	SendSubCommand				; Set the fade out command
...
ObjSonic_SpecialChunks:
	cmpi.b	#3,(v_zone).l				; Are we in Quartz Quadrant?
	beq.s	.HasSpecChunks				; If so, branch
	cmpi.b	#5,(v_zone).l				; Are we in Stardust Speedway?
	beq.s	.HasSpecChunks				; If so, branch
	cmpi.b	#2,(v_zone).l				; Are we in Tidal Tempest?
	beq.s	.HasSpecChunks				; If so, branch
	tst.b	(v_zone).l				; Are we in Palmtree Panic?
...
; -------------------------------------------------------------------------------
.NormalCode:
	cmp.b	(v_loop_chunks+2).w,d1			; Are we in a special roll tunnel?
...
.NotRoll:
	cmp.b	(v_loop_chunks+3).w,d1			; Are we in a regular roll tunnel?
	beq.w	.RollTunnel				; If so, branch

; -------------------------------------------------------------------------------

.CheckIfLoop:
	cmp.b	(v_loop_chunks).w,d1			; Are we on a loop?
	beq.s	.CheckIfLeft				; If so, branch
	cmp.b	(v_loop_chunks+1).w,d1			; Are we on a special loop?
...
ObjSonic_Animate:
	lea	(Ani_Sonic).l,a1			; Get animation script
...
Ani_Sonic:
	include	"level/objects/sonic/anim.asm"
	even
...
LoadSonicDynPLC:
	...
	lea	(v_sonic_last_frame).w,a2		; Get the ID of the last sprite frame that was loaded
	moveq	#0,d0
	...
	lea	(DPLC_Sonic).l,a2			; Get DPLC data for our current sprite frame
	add.w	d0,d0
	adda.w	(a2,d0.w),a2
	...
	lea	(v_sonic_art).w,a3			; Get sprite frame tile buffer
	move.b	#1,(v_sonic_frame_changed).w		; Mark buffer as updated
...
ObjSonic_ChkFlipper:
	...
	move.w	#$CC,d0					; Play spring sound
	jsr	PlayFMSound
...
SendSubCommand:
	...
	move.b	#1,(v_boss_music_playing).l		; Mark boss music as being played

.NotBossMusic:
	move.w	d0,(GA_CMD_0).l				; Send the command

.WaitSubCPU:
	move.w	(GA_STAT_0).l,d0			; Has the Sub CPU received the command?
	beq.s	.WaitSubCPU				; If not, wait
	cmp.w	(GA_STAT_0).l,d0
	bne.s	.WaitSubCPU				; If not, wait

	move.w	#0,(GA_CMD_0).l				; Mark as ready to send commands again

.WaitSubCPUDone:
	move.w	(GA_STAT_0).l,d0			; Is the Sub CPU done processing the command?
	bne.s	.WaitSubCPUDone				; If not, wait
	move.w	(GA_STAT_0).l,d0
...
ObjCheckpoint_SaveData:					; Save some values
	move.b	(v_reset_lvl_flags).l,(v_saved_reset_lvl_flags).l
	move.w	(v_player+oX).w,(v_saved_x).l
	move.w	(v_player+oY).w,(v_saved_y).l
	move.b	(v_water_routine).w,(v_saved_water_rout).l
	move.w	(v_bottom_bound).w,(v_saved_btm_bound).l
	move.w	(v_cam_fg_x).w,(v_saved_cam_fg_x).l
	move.w	(v_cam_fg_y).w,(v_saved_cam_fg_y).l
	move.w	(v_cam_bg_x).w,(v_saved_cam_bg_x).l
	move.w	(v_cam_bg_y).w,(v_saved_cam_bg_y).l
	move.w	(v_cam_bg2_x).w,(v_saved_cam_bg2_x).l
	move.w	(v_cam_bg2_y).w,(v_saved_cam_bg2_y).l
	move.w	(v_cam_bg3_x).w,(v_saved_cam_bg3_x).l
	move.w	(v_cam_bg3_y).w,(v_saved_cam_bg3_y).l
	move.w	(v_water_height2).w,(v_saved_water_height2).l
	move.b	(v_water_routine).w,(v_saved_water_rout).l
	move.b	(v_water_full).w,(v_saved_water_full).l
...
.StoreTime:
	move.l	d0,(v_saved_time).l

	move.b	(v_mini_sonic).l,(v_saved_mini_sonic).l
	rts
...
ObjCheckpoint_Init:
	...
	move.b	(v_last_checkpoint).l,d0		; Has a later checkpoint already been activated?
...
ObjCheckpoint_Main:
	...
	move.b	#1,oChkPntActive(a0)			; Mark as activated
	move.b	oSubtype(a0),(v_last_checkpoint).l	; Set current checkpoint ID to ours

	move.b	#1,(v_reset_lvl_flags).l		; Mark checkpoint as active in the level
	bsr.w	ObjCheckpoint_SaveData			; Save level data at this point

	move.w	#$A1,d0					; Play checkpoint sound
	jmp	PlayFMSound
...
ObjCheckpoint_Animate:
	lea	(Ani_Checkpoint).l,a1			; Animate sprite
	bra.w	AnimateObject
...
ObjUnusedBadnik_Main:
	...
	lea	(Ani_Powerup).l,a1			; Animate sprite
	bsr.w	AnimateObject
	jmp	DrawObject				; Draw sprite
...
ObjExplosion_Main:
	lea	(Ani_Explosion).l,a1			; Animate sprite
	bsr.w	AnimateObject
	jmp	DrawObject				; Draw sprite
...
ObjFlower_Animate:
	lea	(Ani_Flower).l,a1			; Animate sprite
	bra.w	AnimateObject
...
ObjFlower_GetRespawnAddr:
	...
	lea	(v_obj_respawns).l,a1			; Get respawn table entry address
	lea	2(a1,d0.w),a1
	rts
...
ObjFlower_GetPosBuffer:
	...
	lea	(v_flower_pos_buffer).l,a1		; Get flower position table address
	rts
...
ObjWaterfallSplash_Main:
	lea	(Ani_WaterfallSplash).l,a1		; Animate sprite
	bsr.w	AnimateObject
	jmp	DrawObject				; Draw sprite
...
; FUNCTION CHUNK AT 00206774 SIZE 000000D2 BYTES

	lea	(v_player).w,a6
	cmpi.b	#2,(v_reset_lvl_flags).l
	beq.w	TimeTravel_LoadData
	move.b	(v_saved_reset_lvl_flags).l,(v_reset_lvl_flags).l
	move.w	(v_saved_x).l,oX(a6)
	move.w	(v_saved_y).l,oY(a6)
	clr.w	(v_ring_count).l
	clr.b	(v_1up_flags).l
	move.l	(v_saved_time).l,(v_time).l
	move.b	#59,(v_time+3).l
	subq.b	#1,(v_time+2).l
	move.b	(v_saved_water_rout).l,(v_water_routine).w
	move.w	(v_saved_btm_bound).l,(v_bottom_bound).w
	move.w	(v_saved_btm_bound).l,(v_dest_btm_bound).w
	move.w	(v_saved_cam_fg_x).l,(v_cam_fg_x).w
	move.w	(v_saved_cam_fg_y).l,(v_cam_fg_y).w
	move.w	(v_saved_cam_bg_x).l,(v_cam_bg_x).w
	move.w	(v_saved_cam_bg_y).l,(v_cam_bg_y).w
	move.w	(v_saved_cam_bg2_x).l,(v_cam_bg2_x).w
	move.w	(v_saved_cam_bg2_y).l,(v_cam_bg2_y).w
	move.w	(v_saved_cam_bg3_x).l,(v_cam_bg3_x).w
	move.w	(v_saved_cam_bg3_y).l,(v_cam_bg3_y).w
	cmpi.b	#6,(v_zone).l
	bne.s	.NoMini
	move.b	(v_saved_mini_sonic).l,(v_mini_sonic).l

.NoMini:
	cmpi.b	#2,(v_zone).l
	bne.s	.NoWater
	move.w	(v_saved_water_height2).l,(v_water_height2).w
	move.b	(v_saved_water_rout).l,(v_water_routine).w
	move.b	(v_saved_water_full).l,(v_water_full).w

.NoWater:
	tst.b	(v_reset_lvl_flags).l
	bpl.s	.End
	move.w	(v_saved_x).l,d0
	subi.w	#320/2,d0
	move.w	d0,(v_left_bound).w
...
Player_ChooseAngle:
	move.b	(v_secondary_angle).w,d3
	cmp.w	d0,d1
	ble.s	.NoExchange
	move.b	(v_primary_angle).w,d3
	exg	d0,d1
...
.KillEnemy:
	bset	#7,oStatus(a1)
	moveq	#0,d0
	move.w	(v_chain_bonus_counter).w,d0
	addq.w	#2,(v_chain_bonus_counter).w
	cmpi.w	#6,d0
	bcs.s	.CappedChain
	moveq	#6,d0
...
.CappedChain:
	...
	cmpi.w	#$20,(v_chain_bonus_counter).w
	bcs.s	.GivePoints
	move.w	#$3E8,d0
	move.w	#$A,oVar3E(a1)
...
.GivePoints:
	bsr.w	AddPoints
	move.w	#$C1,d0  ; Enemy Destroyed SFX
	jsr	PlayFMSound
	move.b	#$18,oID(a1)
...
KillPlayer:
	tst.w	(v_debug_mode).l
	bne.s	.End
	move.b	#0,(v_invincible).l
...
; -------------------------------------------------------------------------------
Ani_Waterfall:
	include	"level/objects/waterfall/anim.asm"
	even
		
MapSpr_Waterfall:
	include	"level/objects/waterfall/map.asm"
	even
...
DebugMode:
	move.b	(v_ctrl1_held).w,d0
	andi.b	#$F,d0
	bne.s	.Accel
	move.l	#$4000,(v_debug_speed).l
	bra.s	.GotSpeed

; -------------------------------------------------------------------------------

.Accel:
	addi.l	#$2000,(v_debug_speed).l
	cmpi.l	#$80000,(v_debug_speed).l
	bls.s	.GotSpeed
	move.l	#$80000,(v_debug_speed).l

.GotSpeed:
	move.l	(v_debug_speed).l,d0
	btst	#0,(v_ctrl1_held).w
	beq.s	.ChkDown
	sub.l	d0,oY(a0)

.ChkDown:
	btst	#1,(v_ctrl1_held).w
	beq.s	.ChkLeft
	add.l	d0,oY(a0)

.ChkLeft:
	btst	#2,(v_ctrl1_held).w
	beq.s	.ChkRight
	sub.l	d0,oX(a0)

.ChkRight:
	btst	#3,(v_ctrl1_held).w
	beq.s	.SetPos
	add.l	d0,oX(a0)
...
.SetPos:
	...
	move.w	oX(a0),d3
	jsr	GetLevelBlock
	move.w	(a1),(v_debug_block).l
	lea	(DebugItemIndex).l,a2
	btst	#6,(v_ctrl1_pressed).w
	beq.s	.NoInc
	moveq	#0,d1
	move.b	(v_debug_object).l,d1
	addq.b	#1,d1
	cmp.b	(a2),d1
	bcs.s	.NoWrap
	move.b	#0,d1
...
.NoWrap:
	move.b	d1,(v_debug_object).l

.NoInc:
	btst	#7,(v_ctrl1_pressed).w
	beq.s	.NoDec
	moveq	#0,d1
	move.b	(v_debug_object).l,d1
	subq.b	#1,d1
...
.NoDec:
	...
	move.b	$C(a2,d1.w),(v_debug_subtype2).l
	move.b	$B(a2,d1.w),d0
	ori.b	#4,d0
	move.b	d0,oRender(a0)
	move.b	#0,oAnim(a0)
	btst	#5,(v_ctrl1_pressed).w
	beq.s	.NoPlace
	bsr.w	FindObjSlot
	bne.s	.NoPlace
	moveq	#0,d1
...
.NoPlace:
	btst	#4,(v_ctrl1_pressed).w
	beq.s	.NoRevert
	move.b	#0,(v_debug_mode).l
...
; -------------------------------------------------------------------------------
v_debug_speed:	dc.l	$50000
DebugItemIndex:	dc.b	$34 ; 4
	dc.b	0
	dc.b	$35
	dc.b	1
...
LevelObjManager:
	moveq	#0,d0
	move.b	(v_lvl_obj_man_rout).w,d0
...
;	movea.l	a0,a1
;	adda.w	(a0),a0
	move.l	a0,(v_obj_load_addr_right).w
	move.l	a0,(v_obj_load_addr_left).w
	adda.w	2(a1),a1
	move.l	a1,(v_obj_load_addr2_right).w
	move.l	a1,(v_obj_load_addr2_left).w
	lea	(v_obj_respawns).l,a2
	move.w	#$101,(a2)
	moveq	#0,d2
	move.w	(v_cam_fg_x).w,d6
	subi.w	#$80,d6
	bcc.s	.SkipLCap
	moveq	#0,d6
...
.SkipLCap:
	andi.w	#$FF80,d6
	movea.l	(v_obj_load_addr_right).w,a0
...
.FoundRightmost:
	move.l	a0,(v_obj_load_addr_right).w
	movea.l	(v_obj_load_addr_left).w,a0
	subi.w	#$80,d6
...
; -------------------------------------------------------------------------------
LevelObjectIndex:	
    dc.w Objects_PPZ1A-LevelObjectIndex, ObjPos_Null-LevelObjectIndex
	dc.w Objects_PPZ2A-LevelObjectIndex, ObjPos_Null-LevelObjectIndex
	dc.w Objects_PPZ3C-LevelObjectIndex, ObjPos_Null-LevelObjectIndex
	dc.w Objects_PPZ1A-LevelObjectIndex, ObjPos_Null-LevelObjectIndex

	dc.w	$FFFF, 0, 0, 0
Objects_PPZ1A:
	incbin	"level/level/PPZ1/objPos/objects.bin"
	even
;Objects_PPZ1B:
	;incbin	"level/level/PPZ1/objPos/objects_R11B.bin"
	;even
;Objects_PPZ1C:
	;incbin	"level/level/PPZ1/objPos/R11C.bin"
	;even
;Objects_PPZ1D:
	;incbin	"level/level/PPZ1/objPos/R11D.bin"
	;even
Objects_PPZ2A:
	incbin	"level/level/PPZ2/objPos/R12A.bin"
	even
;Objects_PPZ2B:
	;incbin	"level/level/PPZ2/objPos/R12A.bin"
	;even
...
;Objects_PPZ2D:
	...
	;even
Objects_PPZ3C:
	incbin	"level/level/PPZ3/objPos/R13C.bin"
	even	
;Objects_PPZ3D:
	;incbin	"level/level/PPZ3/objPos/R13D.bin"
	;even	
ObjPos_Null:
...
ObjScenery_BaseTileList:
	...
	dc.w	$38F
MapSpr_Scenery:
	include	"level/objects/scenery/map.asm"
	even
; -------------------------------------------------------------------------------
...
ObjHollowLogBG_BaseTileList:
	...
	dc.w	$428, $37F
MapSpr_HollowLogBG:
	include	"level/objects/logbg/map.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
MapSpr_SonicHole:
	include	"level/objects/sonichole/map.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
ObjRotPlatform_Index:dc.w	ObjRotPlatform_Init-ObjRotPlatform_Index
...
.ChkInput:
	move.w	(v_ctrl1_held).w,(v_player_held).w
...
; -------------------------------------------------------------------------------
Ani_RotPlatform:
	include	"level/objects/rotatingptfm/anim.asm"
	even
MapSpr_RotPlatform:
	include	"level/objects/rotatingptfm/map.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
MapSpr_GrayRock:
	include	"level/objects/grayrock/map.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
Ani_S1Spring:
	include	"level/unused/anim/s1spring.asm"
	even
MapSpr_S1Spring:
	include	"level/unused/mapspr/s1spring.asm"
	even
Ani_Spring:
	include	"level/objects/spring/anim.asm"
	even
	include	"level/objects/spring/map.asm"
	even
Ani_MovingSpring:
	include	"level/objects/spring/animwheel.asm"
	even
MapSpr_MovingSpring:
	include	"level/objects/spring/mapwheel.asm"
	even
; -------------------------------------------------------------------------------
...
.DoAnim:
	tst.w	(v_time_stop_timer).l
	bne.s	.Display
	move.b	(v_ring_anim_frame).l,oMapFrame(a0)
...
.NotMMZ:
	...
	move.b	#8,oWidth(a1)
	move.b	#8,oYRadius(a1)
	move.b	#$FF,(v_ring_spill_timer).l
	tst.w	d4
	bmi.s	.SetVel
	move.w	d4,d0
	jsr	CalcSine
...
.DidInit:
	move.w	#0,(v_ring_count).l
	move.b	#$80,(v_update_rings).l
	move.b	#0,(v_1up_flags).l
	move.w	#$C6,d0
	jsr	PlayFMSound
...
; FUNCTION CHUNK AT 0020A0EE SIZE 00000004 BYTES
	...
	move.b	(v_ring_spill_frame).l,oMapFrame(a0)
	bsr.w	ObjMove
	addi.w	#$18,oYVel(a0)
	bmi.s	.ChkDel
...
.ChkDel:
	tst.b	(v_ring_spill_timer).l
	beq.s	ObjLostRing_Destroy
	move.w	(v_bottom_bound).w,d0
	addi.w	#$E0,d0
	cmp.w	oY(a0),d0
...
; -------------------------------------------------------------------------------
Ani_Ring:
	include	"level/objects/ring/anim.asm"
	even
MapSpr_Ring:
	include	"level/objects/ring/map.asm"
	even
MapSpr_S1BigRing:
	include	"level/unused/mapspr/s1bigring.asm"
	even
MapSpr_S1BigRingFlash:
	include	"level/unused/mapspr/s1bigringflash.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
Ani_SmallPlatform:
	include	"level/objects/smallptfm/anim.asm"
	even
MapSpr_SmallPlatform:
	include	"level/objects/smallptfm/map.asm"
	even
; -------------------------------------------------------------------------------
...
ObjMonitorContents_GainInvinc:
	...
	move.b	#3,(v_obj_inv_star1).w
	move.b	#1,(v_obj_inv_star1+oAnim).w
	move.b	#3,(v_obj_inv_star2).w
	move.b	#2,(v_obj_inv_star2+oAnim).w
	move.b	#3,(v_obj_inv_star3).w
	move.b	#3,(v_obj_inv_star3+oAnim).w
	move.b	#3,(v_obj_inv_star4).w
	move.b	#4,(v_obj_inv_star4+oAnim).w
	move.w	#$87,d0 ; Play Invincibility Theme
	jsr	PlayFMSound
	tst.b	(v_time_zone).l
	bne.s	.NotPast
	move.w	#$82,d0
...
.GainSpeedShoes:
	...
	move.w	#$80,(v_sonic_deceleration).w
	move.w	#$84,d0 ; Play Speed Shoes Theme
	jsr	PlayFMSound
	tst.b	(v_time_zone).l
	bne.s	.NotPast
	move.w	#$82,d0
...
; -------------------------------------------------------------------------------
Ani_Monitor:
	include	"level/objects/monitor/anim.asm"
	even
MapSpr_MonitorTimePost:
	include	"level/objects/monitor/map.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
MapSpr_Points:
	include	"level/objects/points/map.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
MapSpr_HUD:
	include	"level/objects/hud/map.asm"
	even
; -------------------------------------------------------------------------------

AddPoints:
	move.b	#1,(v_update_score).l
	lea	(v_score).l,a3
	add.l	d0,(a3)
	move.l	#999999,d1
	cmp.l	(a3),d1
...
.CappedScore:
	move.l	(a3),d0
	cmp.l	(v_next_score_1up).l,d0
	bcs.s	.End
	addi.l	#5000,(v_next_score_1up).l
	addq.b	#1,(v_life_count).l
	addq.b	#1,(v_update_lives).l
	move.w	#$7A,d0
	jmp	SendSubCommand
...
; -------------------------------------------------------------------------------
Ani_Anton:
	include	"level/objects/anton/anim.asm"
	even
MapSpr_Anton:
	include	"level/objects/anton/map.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
Ani_PataBata:
	include	"level/objects/patabata/anim.asm"
	even
MapSpr_PataBata1:
	include	"level/objects/patabata/mapnormal.asm"
	even
MapSpr_PataBata2:
	include	"level/objects/patabata/mapdamaged.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
Ani_Mosqui1:
	include	"level/objects/mosqui/animnormal.asm"
	even
Ani_Mosqui2:
	include	"level/objects/mosqui/animdamaged.asm"
	even
MapSpr_Mosqui1:
	include	"level/objects/mosqui/mapnormal.asm"
	even
MapSpr_Mosqui2:
	include	"level/objects/mosqui/mapdamaged.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
Ani_Tamabboh1:
	include	"level/objects/tamabboh/animnormal.asm"
	even
Ani_Tamabboh2:
	include	"level/objects/tamabboh/animdamaged.asm"
	even
	include	"level/objects/tamabboh/map.asm"
	even
...
; -------------------------------------------------------------------------------
Ani_TamabbohMissile:
	include	"level/objects/tamabboh/animmissile.asm"
	even
MapSpr_TamabbohMissile:
	include	"level/objects/tamabboh/mapmissile.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
Ani_TagaTaga1:
	include	"level/objects/tagataga/animnormal.asm"
	even
Ani_TagaTaga2:
	include	"level/objects/tagataga/animdamaged.asm"
	even
	include	"level/objects/tagataga/map.asm"
	even
...
; -------------------------------------------------------------------------------
Ani_SpringBoard:
	include	"level/objects/springboard/anim.asm"
	even
MapSpr_SpringBoard:
	include	"level/objects/springboard/map.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
MapSpr_Spikes:
	include	"level/objects/spikes/map.asm"
	even
; -------------------------------------------------------------------------------
...
.StandOn:
	jsr	ClearObjRide
	move.w	#$B9,d0 ; Play Collapsing Platform SFX
	jsr	PlayFMSound
	addq.b	#2,oRoutine(a0)
	move.b	oSubtype(a0),d0
...
MapSpr_CollapsePlatform1:
	include	"level/objects/platform/mapcollapseledge.asm"
	even
...
MapSpr_CollapsePlatform3:
	include	"level/objects/platform/mapcollapsepieces.asm"
	even
MapSpr_CollapsePlatform2:
	include	"level/objects/platform/mapcollapse.asm"
	even
...
MapSpr_CollapsePlatform4:
	include	"level/objects/platform/mapcollapsepieces2.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
MapSpr_Platform:
	include	"level/objects/platform/mapnormal.asm"
	even
MapSpr_Platform2:
	include	"level/objects/platform/mapnormal2.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
Ani_FlapDoorV:
	include	"level/objects/flapdoor/animverti.asm"
	even
MapSpr_FlapDoorV:
	include	"level/objects/flapdoor/mapverti.asm"
	even
; -------------------------------------------------------------------------------
...
LoadCameraPLC_Full:
	...
	cmpi.w #$0001,(v_zone).l
	bne.s  .Load
	lea	CameraPLC_Ranges_PPZ2(pc),a1
.Load:	
	moveq	#0,d0
	moveq	#0,d1
	move.w	(v_cam_fg_x).w,d0
...
CameraPLC_Ranges_PPZ2:
    dc.w $A00                 ; DATA XREF: LoadStageData:LoadSectionArt↑o ...
    dc.w $1500
    dc.w $1800
    dc.w $1D00
    dc.w $FFFF
...
LevelObj_SetBaseTile:
	lea	(LevelObj_BaseTileList).l,a1
	add.w	d0,d0
...
; -------------------------------------------------------------------------------
Ani_FlyingAnimal:
	include	"level/objects/animal/animflying.asm"
	even
Ani_GroundAnimal:
	include	"level/objects/animal/animground.asm"
	even
MapSpr_FlyingAnimal:
	include	"level/objects/animal/mapflying.asm"
	even
MapSpr_GroundAnimal:
	include	"level/objects/animal/mapground.asm"
	even
...
ObjBigRing_Main:
	...
	move.b	#1,(v_entered_big_ring).l
	addq.b	#2,oRoutine(a0)
	move.w	(v_cam_fg_x).w,d0
	addi.w	#$150,d0
	move.w	d0,oX(a1)
	bset	#0,(v_ctrl_locked).w
	move.w	#$808,(v_player_held).w
	move.w	#0,oXVel(a1)
	move.w	#0,oPlayerGVel(a1)
...
ObjGoalPost_Main:
	...
	bcs.s	.End
	addq.b	#2,oRoutine(a0)
	move.w	(v_cam_fg_x).w,(v_left_bound).w
	move.w	(v_cam_fg_x).w,(v_dest_left_bound).w
	clr.w	(v_time_warp_timer).w
	clr.b	(v_time_warp_dir).w
	clr.b	(v_time_warp_on).l
	moveq	#$12,d0
	jmp	LoadPLC
...
ObjSignpost_Main:
	...
	move.b	#0,oMapFrame(a0)
	addq.b	#2,oRoutine(a0)
	clr.b	(v_speed_shoes).l
	clr.b	(v_invincible).l
	move.w	#$CF,d0
	jmp	PlayFMSound
...
.NotSSZ3:
	...
	move.b	#$3A,oID(a1)
	move.b	#$10,oVar32(a1)
	move.b	#1,(v_update_bonus_score).w
	moveq	#0,d0
	move.b	(v_time+1).l,d0
	mulu.w	#60,d0
	moveq	#0,d1
	move.b	(v_time+2).l,d1
	add.w	d1,d0
...
.GetBonus:
	add.w	d0,d0
	move.w	TimeBonuses(pc,d0.w),(v_bonus_countdown_1).w
	move.w	(v_ring_count).l,d0
	mulu.w	#$64,d0
	move.w	d0,(v_bonus_countdown_2).w
...
LoadFlowerCapsulePal:
	move.w	#7,d6
	lea	(Pal_FlowerCapsule).l,a1
	lea	(v_palette+$20).w,a2
...
; -------------------------------------------------------------------------------
Pal_FlowerCapsule:
	incbin	"level/objects/flowercapsule/palette.bin"
	even
Ani_BigRingFlash:
	include	"level/objects/bigring/animflash.asm"
	even
MapSpr_BigRingFlash:
	include	"level/objects/bigring/mapflash.asm"
	even
ArtNem_BigRingFlash:
	incbin	"level/objects/bigring/artflash.bin"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
MapSpr_3DPlant:
	include	"level/objects/3dplant/map.asm"
	even
Ani_3DRamp:
	include	"level/objects/3dramp/anim.asm"
	even
; -------------------------------------------------------------------------------
...
; -------------------------------------------------------------------------------
Ani_RobotGenerator:
	include	"level/objects/robotgenerator/anim.asm"
	even
MapSpr_RobotGenerator:
	include	"level/objects/robotgenerator/map.asm"
	even
...
Ani_MetalSonicHologram:
	include	"level/objects/projector/anim.asm"
	even
MapSpr_Projector:
	include	"level/objects/projector/map.asm"
	even
...
ObjGameOver_Init:
	...
	move.b	#8,(v_load_shield_art).l
	bclr	#0,(v_time_over).l
	beq.s	.NotTimeOver
	tst.b	(v_life_count).l
	beq.s	.GameOver
	move.l	#MapSpr_GameOver2,4(a0)
	addq.b	#2,(v_load_shield_art).l
	bra.s	.GameOver
...
.LoadAuxObj:
	...
	move.w	#$8F,d0 ; Play Game Over Theme
	jsr	PlayFMSound
	move.w	#$6E,d0
	jmp	SendSubCommand
...
; -------------------------------------------------------------------------------
	include	"level/objects/gameover/map.asm"
	even
; -------------------------------------------------------------------------------
...
ObjResults_BonusCountdown:
	move.b	#1,(v_update_bonus_score).w
	moveq	#0,d0
	tst.w	(v_bonus_countdown_1).w
	bne.s	.GiveBonus1
	tst.w	(v_bonus_countdown_2).w
	bne.s	.GiveBonus2
	subq.w	#1,oVar32(a0)
	bpl.s	.KeepRout
	addq.b	#2,oRoutine(a0)
...
.GiveBonus1:
	addi.w	#10,d0
	subi.w	#100,(v_bonus_countdown_1).w
	tst.w	(v_bonus_countdown_2).w
	beq.s	.ChkDone

.GiveBonus2:
	addi.w	#10,d0
	subi.w	#100,(v_bonus_countdown_2).w
...
.ChkDone:
	...
	tst.w	(v_bonus_countdown_2).w
	bne.s	.HaveBonus
	move.w	#$C5,d0 ; play "ker-ching" sound
	jsr	PlayFMSound
; =============================================================================
; I dunno what these lines do, but commenting them does not affect anything...
; =============================================================================
	;jsr	StopZ80
...
ObjResults_NextLevel:
	move.w	#2,(v_level_restart).l
	move.b	#0,(v_reset_lvl_flags).l
	clr.w	(v_last_camera_plc).l
	clr.l	(v_flower_count).l
	clr.b	(v_unk_flag).l
	clr.b	(v_destroyed_projector).l
	clr.b	(v_last_checkpoint).l
	tst.b	(v_time_attack_mode).l
	beq.s	.NotTimeAttack
	bclr	#0,(v_load_plc_flags).l

.NotTimeAttack:
	bclr	#1,(v_load_plc_flags).l
	move.b	#1,(v_time_zone).l
	move.w	(v_zone).l,d0
	addq.b	#1,d0
	cmpi.b	#2,d0
	bne.s	.NotAct3
	move.b	#1,(v_time_zone).l ; Set to 2 for future, currently at 1 for present(Due to Broken Timezones)
...
.NotAct1:
	...
	tst.b	(v_good_future).l
	beq.s	.End
	clr.b	(v_good_future).l
	bset	d0,(v_good_future_flags).l
	cmpi.b	#3,(v_good_future_flags).l
	bne.s	.End
...
ObjResults_Data:dc.w	$CC, 0, $120, 0
	...
	include	"level/objects/endresults/map.asm"
	even
...
MapSpr_TitleCard:
	include	"level/objects/titlecard/map.asm"
	even
; -------------------------------------------------------------------------------
...
MapSpr_BreakableWall:
	include	"level/objects/breakablewall/map.asm"
	even
...
LevelDataIndex:
	...
	lhead	0,	0,	0,	MapNem_LevelBlocks,	0,	0,	5	; Palmtree Panic Zone

LevelDataBlocks:
; Palmtree Panic Zone
	            dc.l MapNem_PPZ_P_Blocks ;Past Act 1
				dc.l MapNem_PPZ_P_Blocks ;Past Act 1
                dc.l MapNem_LevelBlocks ;Present Act 1
                dc.l MapNem_PPZ_BF_Blocks ;Future Act 1
                dc.l MapNem_PPZ_GF_Blocks ;Good Future Act 1

	            dc.l PPZ_2_BlocksP ;Past Act 2
                dc.l PPZ_2_Blocks ;Present Act 2
                dc.l PPZ_2_BlocksBF ;Future Act 2
                dc.l PPZ_2_BlocksGF ;Good Future Act 2

	            dc.l PPZ_3_Blocks ;Past Act 3
                dc.l PPZ_3_Blocks ;Present Act 3
                dc.l PPZ_3_Blocks ;Future Act 3
                dc.l PPZ_3_Blocks_BF ;Good Future Act 3	

	            dc.l MapNem_LevelBlocks ;Past Act 4
                dc.l MapNem_LevelBlocks ;Present Act 4
                dc.l MapNem_LevelBlocks ;Future Act 4
                dc.l MapNem_LevelBlocks ;Good Future Act 4			
				even						
LevelPaletteID:
	dc.b	5
	dc.b	5
...
LevelPLCID_Past:
                ...
                even													
PLCIndex:
	dc.w	PLC_Level-PLCIndex        ;0
	dc.w	PLC_Std-PLCIndex          ;1
	dc.w	PLC_Cam1_Full-PLCIndex    ;2
	dc.w	PLC_Level-PLCIndex        ;3
	dc.w	PLC_Cam2_Full-PLCIndex    ;4
	dc.w	PLC_Cam3_Full-PLCIndex    ;5
	dc.w	PLC_Cam4_Full-PLCIndex    ;6
	dc.w	PLC_Cam5_Full-PLCIndex    ;7
	dc.w	PLC_Cam1_Incr-PLCIndex    ;8
	dc.w	PLC_Cam2_Incr-PLCIndex    ;9
	dc.w	PLC_Cam3_Incr-PLCIndex    ;$A
	dc.w	PLC_Cam4_Incr-PLCIndex    ;$B
	dc.w	PLC_Cam5_Incr-PLCIndex    ;$C
	dc.w	PLC_Cam1_Full-PLCIndex    ;$D
	dc.w	PLC_Cam1_Full-PLCIndex    ;$E
	dc.w	PLC_Cam1_Full-PLCIndex    ;$F
	dc.w	PLC_Results-PLCIndex      ;$10
	dc.w	PLC_Cam1_Full-PLCIndex    ;$11
	dc.w	PLC_Signpost-PLCIndex     ;$12
	dc.w	PLC_Title-PLCIndex        ;$13
; Palmtree Panic Zone Act 1
	dc.w	PLC_PPZ_P-PLCIndex        ;$14
	dc.w	PLC_PPZ_F-PLCIndex        ;$15
	dc.w	PLC_PPZ_GF-PLCIndex       ;$16
; Palmtree Panic Zone Act 2
    dc.w	PLC_PPZ2-PLCIndex         ;$17
    dc.w 	PLC_PPZ2_P-PLCIndex       ;$18
    dc.w 	PLC_PPZ2_F-PLCIndex       ;$19
    dc.w 	PLC_PPZ2_GF-PLCIndex      ;$1A
; Palmtree Panic Zone Act 3	
	dc.w	PLC_PPZ3-PLCIndex         ;$1B
    dc.w 	PLC_PPZ3_F-PLCIndex       ;$1C
	dc.w 	PLC_PPZ3_Eggman-PLCIndex       ;$1D
...
PLC_PPZ3_F:	dc.w	0
	dc.l	ArtNem_PPZ_3BF
	dc.w	0
PLC_PPZ3_Eggman:dc.w 2 
    dc.l Art_FlowerCapsule
    dc.w $9020
    dc.l Art_BossMachine
    dc.w $6B20
    dc.l Art_Robotnik
    dc.w $7FA0
...
Blk256_GHZ:
LevelChunks:
	incbin	"level/level/PPZ1/Map256_u/chunks.bin"
	even	
MapSpr_Sonic:
	include	"level/objects/sonic/map.asm"
	even
MapSpr_3DRamp:
	include	"level/objects/3dramp/map.asm"
	even
ArtUnc_Sonic:
	incbin	"level/objects/sonic/art.bin"
	even
DPLC_Sonic:
	include	"level/objects/sonic/dplc.asm"
	even
ArtNem_Points:
	incbin	"level/objects/points/art.bin"
	even
ArtNem_SeedPods:
	incbin	"level/objects/flowercapsule/art.bin"
	even
ArtNem_BigRing:
	incbin	"level/objects/bigring/art.bin"
	even
ArtNem_GoalPost:
	incbin	"level/objects/endpost/artgoal.bin"
	even
ArtNem_Signpost:
	incbin	"level/objects/endpost/artsign.bin"
	even
ArtNem_Results:
	incbin	"level/objects/endresults/art.bin"
	even
ArtUnc_TimeOver:
	incbin	"level/objects/gameover/arttimeover.bin"
	even
ArtUnc_GameOver:
	incbin	"level/objects/gameover/artgameover.bin"
	even
ArtNem_TitleCard:
	incbin	"level/objects/titlecard/artbase.bin"
	even
ArtUnc_Shield:
	incbin	"level/objects/powerup/artshield.bin"
	even
ArtUnc_InvStars:
	incbin	"level/objects/powerup/artinvinc.bin"
	even
ArtUnc_TimeStars:
	incbin	"level/objects/powerup/artwarp.bin"
	even
Art_DiagonalSpring:
ArtNem_DiagonalSpring:
	incbin	"level/objects/spring/artdiag.bin"
	even
ArtNem_Springs:
	incbin	"level/objects/spring/artnormal.bin"
	even
ArtNem_MonitorTimePosts:
	incbin	"level/objects/monitor/art.bin"
	even
ArtNem_Explosions:
	incbin	"level/objects/explosion/art.bin"
	even
ArtNem_Rings:
	incbin	"level/objects/ring/art.bin"
	even
ArtUnc_LifeIcon:
	incbin	"level/objects/hud/artlifeicon.bin"
	even
ArtUnc_HUD:
	incbin	"level/objects/hud/artnumbers.bin"
	even
ArtNem_HUD:
	incbin	"level/objects/hud/artbase.bin"
	even
ArtNem_ContinuePost:
	incbin	"level/objects/checkpoint/art.bin"
	even
ArtNem_Unused1:
	incbin	"level/objects/logbg/artunused.bin"
	even
Art_LogShadow:
ArtNem_HollowLogBG:
	incbin	"level/objects/logbg/art.bin"
	even
Ani_Flower:
	include	"level/objects/flower/anim.asm"
	even
MapSpr_Flower:
	include	"level/objects/flower/map.asm"
	even
ArtNem_Flower:
	incbin	"level/objects/flower/art.bin"
	even
ArtNem_TitleCardText:
	incbin	"level/objects/titlecard/arttext.bin"
	even
ArtNem_CollapsePlatform:
	incbin	"level/objects/platform/art.bin"
	even
Art_Boulder:
ArtNem_GreyRock:
	incbin	"level/objects/grayrock/art.bin"
	even
Art_FloatBlock:
ArtNem_HiddenPlatforms:
	incbin	"level/objects/smallptfm/art.bin"
	even
ArtNem_Wheel:
	incbin	"level/objects/spring/artwheel.bin"
	even
Art_SpinningDisc:
ArtNem_RotPlatform:
	incbin	"level/objects/rotatingptfm/art.bin"
	even
ArtNem_WaterSplash:
	incbin	"level/objects/splash/artwaterfall.bin"
	even
ArtNem_ExtraWaterSplashes1:
	incbin	"level/objects/waterfall/art.bin"
	even
Art_TunnelDoorSplash:
ArtNem_SpinTubeDoors:
	incbin	"level/objects/flapdoor/art.bin"
	even
ArtNem_ExtraWaterSplashes2:
	incbin	"level/objects/splash/artnormal.bin"
	even
Art_Anton:
ArtNem_Anton:
	incbin	"level/objects/anton/art.bin"
	even
Art_Mosqui:
ArtNem_Mosqui:
	incbin	"level/objects/mosqui/art.bin"
	even
Art_PataBata:
ArtNem_PataBata:
	incbin	"level/objects/patabata/art.bin"
	even
Art_TagaTaga:
ArtNem_TagaTaga:
	incbin	"level/objects/tagataga/art.bin"
	even
Art_Tamabboh:
ArtNem_MotorizedBeetle:
	incbin	"level/objects/tamabboh/art.bin"
	even
Art_Springboard:
ArtNem_SprintPole:
	incbin	"level/objects/springboard/art.bin"
	even
ArtNem_Button:
	incbin	"level/unused/art/button.bin"
	even
ArtNem_Spikes:
	incbin	"level/objects/spikes/art.bin"
	even
Art_Swing:
ArtNem_SwingingPlatform:
	incbin	"level/unused/art/swingptfm.bin"
	even
Art_Animals:
ArtNem_PPZAnimals:
	incbin	"level/objects/animal/art.bin"
	even
ArtNem_RotPlatformDrill:
	incbin	"level/unused/art/rotptfmdrill.bin"
	even
Art_RobotGenWithered:
ArtNem_AgedTeleporter:
	incbin	"level/objects/robotgenerator/art.bin"
	even
Art_FlowerCapsule:
Art_BossMachine:                        ; DATA XREF: ROM:0020E3B8↑o
    incbin	"artnem/Art_BossMachine.bin"
    even
Art_Robotnik:                           ; DATA XREF: ROM:0020E3BE↑o
    incbin	"artnem/Art_Robotnik.bin"
    even	
; Main Game Collision Files
ColAngleMap:
	incbin	"level/level/mainCol/colangles.bin"
	even
ColHeightMap:
	incbin	"level/level/mainCol/colheights.bin"
	even
ColWidthMap:
	incbin	"level/level/mainCol/colwidths.bin"
	even
; Palmtree Panic Act 1 Collision Files
LevelCollision:
	incbin	"level/level/PPZ1/Collision/colblocks.bin"
	even
PPZ1_Collision_P:
	incbin	"level/level/PPZ1/Collision/R11B.bin"
	even
PPZ1_Collision_GF:
	incbin	"level/level/PPZ1/Collision/R11C.bin"
	even
PPZ1_Collision_BF:
	incbin	"level/level/PPZ1/Collision/R11D.bin"
	even
; Palmtree Panic Act 2 Collision Files
PPZ2_Collision:
	incbin	"level/level/PPZ2/Collision/R12A.bin"
	even
PPZ2_Collision_P:
	incbin	"level/level/PPZ2/Collision/R12B.bin"
	even
PPZ2_Collision_GF:
	incbin	"level/level/PPZ2/Collision/R12C.bin"
	even
PPZ2_Collision_BF:
	incbin	"level/level/PPZ2/Collision/R12D.bin"
	even
; Palmtree Panic Act 3 Collision Files
PPZ3_Collision_GF:
	incbin	"level/level/PPZ3/Collision/R13C.bin"
	even
PPZ3_Collision_BF:
	incbin	"level/level/PPZ3/Collision/R13D.bin"
	even
LevelLayoutIndex:
	dc.w LevelLayout_FG-LevelLayoutIndex
	dc.w LevelLayout_BG-LevelLayoutIndex
	dc.w byte_68D70-LevelLayoutIndex

	dc.w LevelR12A_FG-LevelLayoutIndex
	dc.w LevelR12A_BG-LevelLayoutIndex
	dc.w byte_68D70-LevelLayoutIndex

	dc.w LevelR13C_FG-LevelLayoutIndex
	dc.w LevelR13C_BG-LevelLayoutIndex
	dc.w byte_68D70-LevelLayoutIndex
	...
	dc.w LevelLayout_Null-LevelLayoutIndex
	dc.w LevelLayout_Null-LevelLayoutIndex
LevelLayoutIndex_Past:
	dc.w LevelR11B_FG-LevelLayoutIndex_Past
	dc.w LevelR11B_BG-LevelLayoutIndex_Past
	dc.w LevelR12B_FG-LevelLayoutIndex_Past
	dc.w LevelR12B_BG-LevelLayoutIndex_Past	
; Palmtree Panic Act 1 Specific Timezone Layouts
LevelLayout_FG:
	incbin	"level/level/PPZ1/FG/levelfg.bin"
	even
LevelLayout_BG:
	incbin	"level/level/PPZ1/BG/levelbg.bin"
	even	
LevelR11B_FG:
	incbin	"level/level/PPZ1/FG/R11B_FG.bin"
	even
LevelR11B_BG:
	incbin	"level/level/PPZ1/BG/R11B_BG.bin"
	even		
LevelR11C_FG:
	incbin	"level/level/PPZ1/FG/R11C_FG.bin"
	even
LevelR11C_BG:
	incbin	"level/level/PPZ1/BG/R11C_BG.bin"
	even	
LevelR11D_FG:
	incbin	"level/level/PPZ1/FG/R11D_FG.bin"
	even
LevelR11D_BG:
	incbin	"level/level/PPZ1/BG/R11D_BG.bin"
	even	

; Palmtree Panic Act 2 Timezone specific layouts
LevelR12A_FG:
	incbin	"level/level/PPZ2/FG/R12A_FG.bin"
	even
LevelR12A_BG:
	incbin	"level/level/PPZ2/BG/R12A_BG.bin"
	even
LevelR12B_FG:
	incbin	"level/level/PPZ2/FG/R12B_FG.bin"
	even
LevelR12B_BG:
	incbin	"level/level/PPZ2/BG/R12B_BG.bin"
	even
LevelR12C_FG:
	incbin	"level/level/PPZ2/FG/R12C_FG.bin"
	even
LevelR12C_BG:
	incbin	"level/level/PPZ2/BG/R12C_BG.bin"
	even
LevelR12D_FG:
	incbin	"level/level/PPZ2/FG/R12D_FG.bin"
	even
LevelR12D_BG:
	incbin	"level/level/PPZ2/BG/R12D_BG.bin"
	even
; Palmtree Panic Act 3 Timezone specific layouts
LevelR13C_FG:
	incbin	"level/level/PPZ3/FG/R13C_FG.bin"
	even
LevelR13C_BG:
	incbin	"level/level/PPZ3/BG/R13C_BG.bin"
	even
LevelR13D_FG:
	incbin	"level/level/PPZ3/FG/R13D_FG.bin"
	even
LevelR13D_BG:
	incbin	"level/level/PPZ3/BG/R13D_BG.bin"
	even
...
ArtNem_SonicHole:
	incbin	"level/objects/sonichole/art.bin"
	even
ArtNem_StarBush:
	incbin	"level/objects/3dplant/art.bin"
	even
ArtNem_LargeFan:
	incbin	"level/objects/3dramp/art.bin"
	even
Ani_Powerup:
	include	"level/objects/powerup/anim.asm"
	even
MapSpr_Powerup:
	include	"level/objects/powerup/map.asm"
	even
Ani_WaterSplash:
	include	"level/objects/splash/animnormal.asm"
	even
MapSpr_WaterSplash:
	include	"level/objects/splash/mapnormal.asm"
	even
Ani_FlapDoorH:
	include	"level/objects/flapdoor/animhoriz.asm"
	even
MapSpr_FlapDoorH:
	include	"level/objects/flapdoor/maphoriz.asm"
	even
Ani_WaterfallSplash:
	include	"level/objects/splash/animwaterfall.asm"
	even
MapSpr_WaterfallSplash:
	include	"level/objects/splash/mapwaterfall.asm"
	even
Ani_Explosion:
	include	"level/objects/explosion/anim.asm"
	even
MapSpr_Explosion:
	include	"level/objects/explosion/map.asm"
	even
Ani_Checkpoint:
	include	"level/objects/checkpoint/anim.asm"
	even
MapSpr_Checkpoint:
	include	"level/objects/checkpoint/map.asm"
	even
Ani_BigRing:
	include	"level/objects/bigring/anim.asm"
	even
MapSpr_BigRing:
	include	"level/objects/bigring/map.asm"
	even
MapSpr_GoalPost_Signpost:
	include	"level/objects/endpost/map.asm"
	even
MapSpr_FlowerCapsule:
	include	"level/objects/flowercapsule/map.asm"
	even
Ani_FlowerCapsule:
	include	"level/objects/flowercapsule/anim.asm"
	even
ArtNem_Projector:
	incbin	"level/objects/projector/art.bin"
	even
ArtNem_AmyRose:
	incbin	"level/objects/amyrose/art.bin"
	even
MapSpr_AmyRose:	
	include	"level/objects/amyrose/map.asm"
	even
Ani_AmyRose:
	include	"level/objects/amyrose/anim.asm"
	even
; Palmtree Panic Act 1 Tiles
Nem_GHZ_1st:
ArtNem_LevelArt:
	incbin	"level/level/PPZ1/Tiles/art.bin"
	even
ArtNem_PPZ_P:
    incbin "level/level/PPZ1/Tiles/art_p.bin"	
    even
ArtNem_PPZ_GF:
    incbin "level/level/PPZ1/Tiles/R11C.bin"	
    even	
ArtNem_PPZ_F:
    incbin "level/level/PPZ1/Tiles/R11D.bin"	
    even
; Palmtree Panic Act 1 Chunks (map256_u)
PPZ_ChunksP:
    incbin	"level/level/PPZ1/Map256_u/chunks_p.bin"
	even
PPZ_ChunksGF:
    incbin	"level/level/PPZ1/Map256_u/R11C.bin"
	even
PPZ_ChunksBF:
    incbin	"level/level/PPZ1/Map256_u/R11D.bin"
	even
; Palmtree Panic Act 1 Blocks	
Blk16_GHZ:
MapNem_LevelBlocks:
	incbin	"level/level/PPZ1/Map16/blocks.bin"
	even
MapNem_PPZ_P_Blocks:
	incbin	"level/level/PPZ1/Map16/blocks_p.bin"
	even
MapNem_PPZ_GF_Blocks:
	incbin	"level/level/PPZ1/Map16/R11C.bin"
	even
MapNem_PPZ_BF_Blocks:
	incbin	"level/level/PPZ1/Map16/R11D.bin"
	even
; Palmtree Panic Act 2 Tiles
ArtNem_PPZ_2:
    incbin "level/level/PPZ2/Tiles/R12A.bin"
    even
ArtNem_PPZ_2P:
    incbin "level/level/PPZ2/Tiles/R12B.bin"
    even
ArtNem_PPZ_2GF:
    incbin "level/level/PPZ2/Tiles/R12C.bin"
    even
ArtNem_PPZ_2BF:
    incbin "level/level/PPZ2/Tiles/R12D.bin"
    even
; Palmtree Panic Act 2 Chunks (map256_u)
PPZ_2_Chunks:
    incbin	"level/level/PPZ2/map256_u/R12A.bin"
	even
PPZ_2_ChunksP:
    incbin	"level/level/PPZ2/map256_u/R12B.bin"
	even
PPZ_2_ChunksGF:
    incbin	"level/level/PPZ2/map256_u/R12C.bin"
	even
PPZ_2_ChunksBF:
    incbin	"level/level/PPZ2/map256_u/R12D.bin"
	even
; Palmtree Panic Act 2 Blocks (map16)
PPZ_2_Blocks:
    incbin	"level/level/PPZ2/Map16/R12A.bin"
	even
PPZ_2_BlocksP:
    incbin	"level/level/PPZ2/Map16/R12B.bin"
	even
PPZ_2_BlocksGF:
    incbin	"level/level/PPZ2/Map16/R12C.bin"
	even
PPZ_2_BlocksBF:
    incbin	"level/level/PPZ2/Map16/R12D.bin"
	even
; Palmtree Panic Act 3 Tiles
ArtNem_PPZ_3:
    incbin "level/level/PPZ3/Tiles/R13C.bin"
    even
ArtNem_PPZ_3BF:
    incbin "level/level/PPZ3/Tiles/R13D.bin"
    even
; Palmtree Panic Act 3 Chunks (map256_u)
PPZ_3_Chunks:
    incbin	"level/level/PPZ3/map256_u/R13C.bin"
	even
PPZ_3_Chunks_BF:
    incbin	"level/level/PPZ3/map256_u/R13D.bin"
	even
; Palmtree Panic Act 3 Blocks (map16)
PPZ_3_Blocks:
    incbin	"level/level/PPZ3/Map16/R13C.bin"
	even		
PPZ_3_Blocks_BF:
    incbin	"level/level/PPZ3/Map16/R13D.bin"
	even
PPZ_Eggman:
   include "_incObj/PPZ_Eggman.asm"	
PPZ3_Scroll_Events:
    include "_inc/PPZ3_Scroll.asm"   
PPZ2_Scroll_Events:
    include "_inc/PPZ2_Scroll.asm"	
TimeTravel:
          rts
;SegaScreen:
   include "_inc/SegaScreen.asm"
;TitleScreen:
   include "_inc/TitleScreen.asm"
; -------------------------------------------------------------------------------
; Leftover data from other level files used as padding, can be replaced
; with a "align $40000"
; -------------------------------------------------------------------------------
...
Sound_Driver: include "Sound Driver.asm"
Sound_Driver_End:
	even
	
SoundDriver_2Load:
	move.w	#$100,d0
	move.w	d0,(Z80_BUS).l				; Stop the Z80
	move.w	d0,(Z80_RESET).l			; Disable Z80 reset

@Wait:
	btst	#0,(Z80_BUS).l
	bne.s	@Wait

	lea	SoundDriver_2,a0				; Load sound driver
	lea	Z80_RAM,a1
	move.w	#SoundDriver_2_End-SoundDriver_2-1,d1

@Load:
	move.b	(a0)+,(a1)+
	dbf	d1,@Load

	moveq	#0,d1					; Reset Z80
	...
	nop
	move.w	d0,(Z80_RESET).l			; Disable Z80 reset
	move.w	d1,(Z80_BUS).l				; Start the Z80
	rts

SoundDriver_2: incbin "Sound Driver SCD.bin"
SoundDriver_2_End:
	even
; ==============================================================
; --------------------------------------------------------------
